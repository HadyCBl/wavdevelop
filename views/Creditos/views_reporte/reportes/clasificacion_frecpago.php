...
// Columna REGION (solo si se filtra por región o por agencia específica)
$mostrarRegionCol = false;
$regionNombre = '';
$agenciaIdSeleccionada = ($radios[0] ?? null) === 'anyofi' ? (int)($selects[0] ?? 0) : 0;

// Compatibilidad: región viene anexada al final de selects/radios
$regionRadio = null;
$regionId = 0;
if (is_array($radios) && count($radios) > 0) {
    $candidateRadio = $radios[count($radios) - 1];
    if ($candidateRadio === 'anyregion') {
        $regionRadio = $candidateRadio;
        $regionId = isset($selects[count($selects) - 1]) ? (int)$selects[count($selects) - 1] : 0;
    }
}
if ($regionRadio === 'anyregion' && $regionId <= 0) {
    echo json_encode(['status' => 0, 'mensaje' => 'Seleccionar Región']);
...
try {
    ...
    if (!$showmensaje) {
        ...
        error_log("Error en clasificacion_frecpago.php: " . $codigoError);
    }
...
switch ($tipo) {
    case 'xlsx';
        printxls($result, $titlereport, $archivo[0], $filtrofecha, $mostrarRegionCol, $regionNombre);
        break;
    case 'pdf':
        printpdf($result, [$titlereport], $info, $filtrofecha, $mostrarRegionCol, $regionNombre);
        break;
}

//funcion para generar pdf
function printpdf($registro, $datos, $info, $filtrofecha, $mostrarRegionCol = false, $regionNombre = '')
{
    ...
    foreach ($registro as $fila) {
        ...
        $ntipperc = $fila['NtipPerC'];
        $numcuotas = $fila['numcuotas'];
        $fechades = $fila['DFecDsbls'];
        $filtrofecha_calc = $filtrofecha; // Usar la fecha del parámetro
        
        // SALDO DE CAPITAL A LA FECHA (igual que cartera_fondos)
        $salcap = ($monto - $cappag);
        $salcap = ($salcap > 0) ? $salcap : 0;
        
        // CAPITAL EN MORA A LA FECHA - CORRECCIÓN SEGÚN FRECUENCIA
        // Temporalmente usar el cálculo original para debug
        $capmora = $capcalafec - $cappag;
        $capmora = ($capmora > 0) ? $capmora : 0;
        
        /* COMENTADO TEMPORALMENTE PARA DEBUG
        $capcalafec_corregido = calcularCapitalAFechaPorFrecuencia(
            $monto, 
            $numcuotas, 
            $fechades, 
            $filtrofecha_calc, 
        ...
        // Usar el capital calculado corregido para mora
        $capmora = $capcalafec_corregido - $cappag;
        $capmora = ($capmora > 0) ? $capmora : 0;
        
        // Para depuración: comparar con el cálculo original
        $capmora_original = $capcalafec - $cappag;
        $capmora_original = ($capmora_original > 0) ? $capmora_original : 0;
        */
    ...
    $pdf->CellFit($ancho_linea2 *2, $tamanio_linea , 'Número de frecuencias: ' . count($frecuencias), 'T', 0, 'L', 0, '', 1, 0);
...
// Función para calcular capital a fecha según frecuencia de pago
function calcularCapitalAFechaPorFrecuencia($monto, $numcuotas, $fechades, $fechacalculo, $ntipperc, $frecuencia) {
    try {
        if ($numcuotas <= 0 || $monto <= 0) return 0;
        
        $capitalPorCuota = $monto / $numcuotas;
        
        // Validar fechas
        if (empty($fechades) || empty($fechacalculo)) return 0;
        
        $fechaDesembolso = new DateTime($fechades);
        $fechaCalculo = new DateTime($fechacalculo);
        
        // Si la fecha de cálculo es anterior al desembolso, no hay capital devengado
        if ($fechaCalculo < $fechaDesembolso) {
            return 0;
        }
        
        $cuotasVencidas = 0;
        
        // Calcular cuotas vencidas según el tipo de período
        switch ((int)$ntipperc) {
            case 1: // Diario
                $diasTranscurridos = $fechaDesembolso->diff($fechaCalculo)->days;
                $cuotasVencidas = min($diasTranscurridos, $numcuotas);
                break;
                
            case 2: // Semanal  
                $semanasTranscurridas = floor($fechaDesembolso->diff($fechaCalculo)->days / 7);
                $cuotasVencidas = min($semanasTranscurridas, $numcuotas);
                break;
                
            case 3: // Quincenal
                $quincenasTranscurridas = floor($fechaDesembolso->diff($fechaCalculo)->days / 15);
                $cuotasVencidas = min($quincenasTranscurridas, $numcuotas);
                break;
                
            case 4: // Mensual
                $fechaTemporal = clone $fechaDesembolso;
                $mesesCompletos = 0;
                
                while ($fechaTemporal <= $fechaCalculo && $mesesCompletos < $numcuotas) {
                    $fechaTemporal->add(new DateInterval('P1M'));
                    if ($fechaTemporal <= $fechaCalculo) {
                        $mesesCompletos++;
                    }
                }
                $cuotasVencidas = $mesesCompletos;
                break;
                
            case 5: // Bimestral
                $fechaTemporal = clone $fechaDesembolso;
                $bimestresCompletos = 0;
                
                while ($fechaTemporal <= $fechaCalculo && $bimestresCompletos < $numcuotas) {
                    $fechaTemporal->add(new DateInterval('P2M'));
                    if ($fechaTemporal <= $fechaCalculo) {
                        $bimestresCompletos++;
                    }
                }
                $cuotasVencidas = $bimestresCompletos;
                break;
            ...
            case 6: // Trimestral
                ...
                while ($fechaTemporal <= $fechaCalculo && $trimestresCompletos < $numcuotas) {
                    $fechaTemporal->add(new DateInterval('P3M'));
                    if ($fechaTemporal <= $fechaCalculo) {
                        $trimestresCompletos++;
                    }
                }
                $cuotasVencidas = $trimestresCompletos;
                break;
                
            case 7: // Semestral
                $fechaTemporal = clone $fechaDesembolso;
                $semestresCompletos = 0;
                
                while ($fechaTemporal <= $fechaCalculo && $semestresCompletos < $numcuotas) {
                    $fechaTemporal->add(new DateInterval('P6M'));
                    if ($fechaTemporal <= $fechaCalculo) {
                        $semestresCompletos++;
                    }
                }
                $cuotasVencidas = $semestresCompletos;
                break;
            ...
            case 8: // Anual
                ...
                while ($fechaTemporal <= $fechaCalculo && $aniosCompletos < $numcuotas) {
                    $fechaTemporal->add(new DateInterval('P1Y'));
                    if ($fechaTemporal <= $fechaCalculo) {
                        $aniosCompletos++;
                    }
                }
                $cuotasVencidas = $aniosCompletos;
                break;
                
            default: // Si no se reconoce el tipo, usar cálculo diario como fallback
                $diasTranscurridos = $fechaDesembolso->diff($fechaCalculo)->days;
                $cuotasVencidas = min($diasTranscurridos, $numcuotas);
        }
        
        // No puede ser mayor al número total de cuotas
        $cuotasVencidas = min($cuotasVencidas, $numcuotas);
        
        return $capitalPorCuota * $cuotasVencidas;
        
    } catch (Exception $e) {
        error_log("Error en calcularCapitalAFechaPorFrecuencia: " . $e->getMessage());
        return 0;
    }
}

//funcion para generar archivo excel
function printxls($registro, $titlereport, $usuario, $filtrofecha, $mostrarRegionCol = false, $regionNombre = '')
{
    require '../../../../vendor/autoload.php';
    ...
    while ($fila < count($registro)) {
        ...
        $ntipperc = $registro[$fila]["NtipPerC"];
        $frecuencia_desc = $registro[$fila]["frecuencia"];
        ...
        */
        ...
        if ($mostrarRegionCol) {
            $activa->setCellValueExplicit('AT' . $i, $regionNombre, \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
        }

            $activa->getStyle("A" . $i . ":" . ($mostrarRegionCol ? 'AT' : 'AS') . $i)->getFont()->setName($fuente);
    ...
    $activa->getStyle("A" . $i . ":" . ($mostrarRegionCol ? 'AT' : 'AS') . $i)->getFont()->setSize($tamanioTabla)->setName($fuente)->setBold(true);
    ...
    $activa->getStyle("A" . $i . ":" . ($mostrarRegionCol ? 'AT' : 'AS') . $i)->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB('FFFF00');
    
    //alinear a la derecha los totales
    $columnas = range('A', $mostrarRegionCol ? 'AT' : 'AS');
