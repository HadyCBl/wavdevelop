...
// Región de origen (solo cuando aplica)
...
function printpdf($registro, $datos, $info, $mostrarRegionCol = false, $regionNombre = '')
{
    ...
    while ($fila < count($registro)) {
        ...
        $nombre = Utf8::decode(mb_convert_case($registro[$fila]["short_name"], MB_CASE_TITLE, 'UTF-8'));
        ...
        if ($codgrupo != $auxgrupo && $auxgrupo != 0 && $fila > 0) {
            $pdf->Ln(1);
            $pdf->SetFont($fuente, 'B', 7);
            $pdf->CellFit($ancho_linea2 * 5 + 15, $tamanio_linea + 1, 'SUBTOTAL GRUPO: ' . $resumenBloqueGrupo['cantidad'] . Utf8::decode(' créditos'), 'T', 0, 'R', 0, '', 1, 0);
            $pdf->CellFit($ancho_linea2, $tamanio_linea + 1, number_format($resumenBloqueGrupo['montos'], 2, '.', ','), 'T', 0, 'R', 0, '', 1, 0);
            $pdf->CellFit($ancho_linea2, $tamanio_linea + 1, number_format($resumenBloqueGrupo['cappag'], 2, '.', ','), 'T', 0, 'R', 0, '', 1, 0);
            $pdf->CellFit($ancho_linea2, $tamanio_linea + 1, number_format($resumenBloqueGrupo['intpag'], 2, '.', ','), 'T', 0, 'R', 0, '', 1, 0);
            $pdf->CellFit($ancho_linea2 - 2, $tamanio_linea + 1, number_format($resumenBloqueGrupo['morpag'], 2, '.', ','), 'T', 0, 'R', 0, '', 1, 0);
            $pdf->CellFit($ancho_linea2, $tamanio_linea + 1, number_format($resumenBloqueGrupo['salcap'], 2, '.', ','), 'T', 0, 'R', 0, '', 1, 0);
            $pdf->CellFit($ancho_linea2, $tamanio_linea + 1, number_format($resumenBloqueGrupo['salint'], 2, '.', ','), 'T', 0, 'R', 0, '', 1, 0);
            $pdf->CellFit($ancho_linea2, $tamanio_linea + 1, number_format($resumenBloqueGrupo['salcap'] + $resumenBloqueGrupo['salint'], 2, '.', ','), 'T', 0, 'R', 0, '', 1, 0);
            $pdf->CellFit($ancho_linea2, $tamanio_linea + 1, number_format($resumenBloqueGrupo['capmora'], 2, '.', ','), 'T', 0, 'R', 0, '', 1, 0);
            $pdf->Cell($ancho_linea2 / 2, $tamanio_linea + 1, '', 'T', 1, 'R');
            $pdf->SetFont($fuente, '', 7);
        ...
        if ($idfondos != $aux && $aux != 0) {
            ...
            $pdf->SetFillColor(255, 255, 153); // Color amarillo claro
            $pdf->CellFit($ancho_linea2 * 5 + 15, $tamanio_linea + 2, 'TOTAL FONDO: ' . $resumenBloqueFondo['cantidad'] . Utf8::decode(' créditos'), 'TB', 0, 'R', true, '', 1, 0);
            $pdf->CellFit($ancho_linea2, $tamanio_linea + 2, number_format($resumenBloqueFondo['montos'], 2, '.', ','), 'TB', 0, 'R', true, '', 1, 0);
            $pdf->CellFit($ancho_linea2, $tamanio_linea + 2, number_format($resumenBloqueFondo['cappag'], 2, '.', ','), 'TB', 0, 'R', true, '', 1, 0);
            $pdf->CellFit($ancho_linea2, $tamanio_linea + 2, number_format($resumenBloqueFondo['intpag'], 2, '.', ','), 'TB', 0, 'R', true, '', 1, 0);
            $pdf->CellFit($ancho_linea2 - 2, $tamanio_linea + 2, number_format($resumenBloqueFondo['morpag'], 2, '.', ','), 'TB', 0, 'R', true, '', 1, 0);
            $pdf->CellFit($ancho_linea2, $tamanio_linea + 2, number_format($resumenBloqueFondo['salcap'], 2, '.', ','), 'TB', 0, 'R', true, '', 1, 0);
            $pdf->CellFit($ancho_linea2, $tamanio_linea + 2, number_format($resumenBloqueFondo['salint'], 2, '.', ','), 'TB', 0, 'R', true, '', 1, 0);
            $pdf->CellFit($ancho_linea2, $tamanio_linea + 2, number_format($resumenBloqueFondo['salcap'] + $resumenBloqueFondo['salint'], 2, '.', ','), 'TB', 0, 'R', true, '', 1, 0);
            $pdf->CellFit($ancho_linea2, $tamanio_linea + 2, number_format($resumenBloqueFondo['capmora'], 2, '.', ','), 'TB', 0, 'R', true, '', 1, 0);
            $pdf->Cell($ancho_linea2 / 2, $tamanio_linea + 2, '', 'TB', 1, 'R', true);
        ...
        if ($codgrupo != $auxgrupo) {
            ...
            $pdf->Cell($ancho_linea2 * 2, 5, ($tipoenti == 'GRUP') ? $groupCounter . ') GRUPO: ' : 'INDIVIDUALES ', '', 0, 'R');
            $pdf->Cell(0, 5, strtoupper($grupo), '', 1, 'L');
            $pdf->SetFont($fuente, '', 7);
            $auxgrupo = $codgrupo;
        }

        // IMPRIMIR LÍNEA DE CRÉDITO
        $pdf->CellFit($ancho_linea2, $tamanio_linea + 1, $cuenta, '', 0, 'L', 0, '', 1, 0);
        // $pdf->Cell($ancho_linea2, $tamanio_linea + 1, $cuenta, '', 0, 'L', 0, '', 1, 0);
        $pdf->CellFit($ancho_linea2 * 2 + 19, $tamanio_linea + 1, $nombre, '', 0, 'L', 0, '', 1, 0);
    ...
    if ($resumenBloqueFondo['cantidad'] > 0) {
        $pdf->Ln(2);
        $pdf->SetFont($fuente, 'B', 8);
    ...
    foreach ($idproductos as $idp) {
        ...
        $pdf->CellFit($ancho_linea2 * 4, 5, karely(strtoupper(Utf8::decode($idp))), '', 0, 'L', 0, '', 1, 0);
...
function printxls($registro, $titlereport, $usuario, $mostrarRegionCol = false, $regionNombre = '')
{
    ...
    while ($fila < count($registro)) {
        ...
        $dictamen = $registro[$fila]["Dictamen"];
        //SALDO DE CAPITAL A LA FECHA
        $salcap = ($monto - $cappag);
        $salcap = ($salcap > 0) ? $salcap : 0;
        ...
        // IMPRIMIR DATOS DEL CRÉDITO - OPCIÓN COMPACTA
        $valoresCredito = [
            $cuenta,
            $nombrefondos,
            $codcliente,
            ...
            strtoupper($nombre),
            $direccion,
            $registro[$fila]["municipio_reside"],
            $tel1,
            $tel2,
            $fechades,
            $fechaven,
            ...
            $ncuotas,
            $fallas,
            $registro[$fila]["sectorEconomico"],
            $registro[$fila]["actividadEconomica"],
            $registro[$fila]["tipocredito"],
            $registro[$fila]["tipo_garantia"],
            $registro[$fila]["ES_PEP"],
            $registro[$fila]["ES_CEP"],
            $dictamen
        ];

        if ($mostrarRegionCol) {
            $valoresCredito[] = $regionNombre;
        }

        ExcelHelper::setRowValues($activa, 1, $i, $valoresCredito);
