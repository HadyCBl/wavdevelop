...
//-----------------------------
$strquery = "SELECT 
IFNULL(ge.nombre, 'SIN ESPECIFICAR') AS nombre,
COUNT(*) AS cantidad,
...
GROUP BY IFNULL(ge.id, 0), IFNULL(ge.nombre, 'SIN ESPECIFICAR')
ORDER BY IFNULL(ge.nombre, 'SIN ESPECIFICAR')";
//--------------------------------
...
function printpdf($registro, $datos, $info, $no_creditos_global,$mora_global,$total_kap,$total_mora,$total_Cmora, $mostrarRegionCol = false, $regionNombre = '' )
{
    ...
    class PDF extends FPDF
    {
        ...
        function Header()
        {
            ...
            $this->Cell(0, 5, 'CLASIFICACION POR ETNIA ' . $this->datos[0], 0, 1, 'C', true);
            $this->Ln(2);
            ...
            $this->Cell($ancho_linea * 2 , 5, 'Etnias', 'B', 0, 'L');
...
function printxls($registro, $titlereport, $usuario, $mostrarRegionCol = false, $regionNombre = '')
{
    ...
    $encabezado_tabla = ["ETNIA", "NO. DE CREDITOS", "SALDO CAPITAL", "PORCENTAJE", "CREDITOS MOROSOS", "SALDO CAPITAL EN MORA", "PORCENTAJE MORA"];
    if ($mostrarRegionCol) {
