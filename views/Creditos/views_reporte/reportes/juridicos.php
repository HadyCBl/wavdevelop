...
WHERE (cremi.CESTADO='J') " . $filfondo . $filagencia . $filtroregion . " 
...
function fpdf_text($value)
{
    ...
    // FPDF trabaja típicamente en ISO-8859-1 (Win-1252 compatible).
    if (function_exists('mb_convert_encoding')) {
        return mb_convert_encoding($value, 'ISO-8859-1', 'UTF-8');
    }
    if (function_exists('iconv')) {
        $out = iconv('UTF-8', 'ISO-8859-1//TRANSLIT', $value);
...
function printpdf($registro, $datos, $info)
{
    ...
    class PDF extends FPDF
    {
        ...
        function Header()
        {
            ...
            $this->Cell(0, 5, 'CARTERA JURÍDICA ' . $this->datos[0], 0, 1, 'C', true);
            $this->Ln(2);
    ...
    while ($fila < count($registro)) {
        ...
        $nombre = fpdf_text($registro[$fila]["short_name"]);
    ...
    foreach ($idproductos as $idp) {
        ...
        $pdf->CellFit($ancho_linea2 * 4, 5, strtoupper(fpdf_text($idp)), '', 0, 'L', 0, '', 1, 0);
...
function printxls($registro, $titlereport, $usuario)
{
    ...
    $activa->setTitle("Cartera Jurídica");
    ...
    $activa->setCellValue("A5", strtoupper("CARTERA JURÍDICA " . $titlereport));
    ...
    $opResult = array(
        'status' => 1,
        'mensaje' => 'Reporte generado correctamente',
        'namefile' => "Cartera Jurídica " . $titlereport,
        'tipo' => "vnd.ms-excel",
