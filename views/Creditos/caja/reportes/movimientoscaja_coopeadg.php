                ...
                function printpdf( $cod2, $movimiento, $nombreUsuario, $detaMovimiento = [],$info = [])
                {
                    class PDF extends FPDF
                    {
                        ...
                        function Header()
                        {
                            ...
                            $this->SetFont('Arial', 'B', 7);
                            $this->Cell(40, 3, 'Usuario: ', 0, 0, 'L');

                            $this->SetFont('Arial', '', 7);
                            $this->Cell(0, 3, decode_utf8($this->nombreUsuario), 0, 1, 'L');
                            ...
                            // $this->SetFont('Arial', 'B', 7);
                            // $this->Cell(40, 3, decode_utf8('No. de Transacción: '), 0, 0, 'L');
                            // $this->SetFont('Arial', '', 7);
                            // $this->Cell(0, 3, $this->cod2, 0, 1, 'L');
                            $this->SetFont('Arial', 'B', 7);
                            $this->Cell(40, 3, 'Fecha y hora de Solicitud: ', 0, 0, 'L');
                            $this->SetFont('Arial', '', 7);
                            $this->Cell(0, 3, $this->movimiento[0]['created_at'], 0, 1, 'L');
                            $this->SetFont('Arial', 'B', 7);
                            $this->Cell(40, 3, decode_utf8('Estado: '), 0, 0, 'L');
                            $this->SetFont('Arial', '', 7);

                            switch ($this->movimiento[0]['estado']){
                                case '0':
                                    $this->Cell(0, 3, 'Rechazado en fecha ' . $this->movimiento[0]['updated_at'], 0, 1, 'L');
                                    break;
                                case '1':
                                    $this->Cell(0, 3, 'Solicitud', 0, 1, 'L');
                                    break;
                                case '2':
                                    $this->Cell(0, 3, 'Aprobado en fecha ' . $this->movimiento[0]['updated_at'], 0, 1, 'L');
                                    break;
                            }
                            $this->SetFont('Arial', 'B', 7);
                            $this->Cell(40, 3, 'Monto Total: ', 0, 0, 'L');
                            $this->SetFont('Arial', '', 7);
                            $this->Cell(0, 3, 'Q. ' . $this->movimiento[0]['total'], 0, 1, 'L');
                            $this->Ln(3);
                    ...
                    function TableData($detaMovimiento = [])
                    {
                        ...
                        if($this->movimiento[0]['detalle'] == 1){
                            ...
                            $this->Cell($w[0], 5, decode_utf8('Denominación'), 1, 0, 'C', true);
                            $this->Cell($w[1], 5, 'Cantidad', 1, 0, 'C', true);
                            $this->Cell($w[2], 5, 'Total', 1, 1, 'C', true);
                            ...
                            while ($i < $totalElementos) {
                                ...
                                if($detaMovimiento[$i]['tipo'] == 1){
                                    $this->Cell($w[0], 5, $abr . ' - ' . $monto . " - Billete", 1, 0, 'C', true);
                                }else{
                                    $this->Cell($w[0], 5, $abr . ' - ' . $monto . " - Moneda", 1, 0, 'C', true);
                                }

                                $this->Cell($w[1], 5, $cantidad, 1, 0, 'C', true);
                                $this->Cell($w[2], 5, number_format($resultado, 2), 1, 1, 'C', true);
                            ...
                            $this->Cell($w[0] + $w[1], 5, 'Total General', 1, 0, 'C', true);
                            $this->Cell($w[2], 5, number_format($totalGeneral, 2), 1, 1, 'C', true);
                        } else {
                            $this->Ln(5);
                            $this->SetFont('Arial', 'B', 8);
