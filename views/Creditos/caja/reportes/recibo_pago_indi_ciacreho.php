...
function reciboCiacreho($pdf, $registro, $hoy, $usuario, $info)
{
    ...
    $pdf->SetFont($fuente, 'B', 11);
    $pdf->CellFit($ancho_linea2 * 2, $tamanio_linea + 2, "  ", 0, 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 + 75, $tamanio_linea + 1, ' ' . (Utf8::decode(mb_strtoupper($registro[0][2], 'utf-8'))), 0, 0, 'C', 0, '', 1, 0);
    $pdf->SetFont($fuente, '', 11);
    $pdf->ln(8);
    //CANTIDAD EN LETRAS
    $pdf->SetFont($fuente, 'B', 11);
    ...
    $res = ($decimal[1] == 0) ? 0 : $decimal[1];
    $pdf->Ln(1);
    $pdf->CellFit($ancho_linea2 * 2 + 14, $tamanio_linea + 2, Utf8::decode($format_monto->toMoney($decimal[0])) . $res . "/100", 0, 0, 'C', 0, '', 1, 0);
    ...
    $pdf->CellFit($ancho_linea2 * 2, $tamanio_linea + 2, 'USUARIO: ' . Utf8::decode(mb_strtoupper($usuario, 'utf-8')), 0, 0, 'L', 0, '', 1, 0);
}
