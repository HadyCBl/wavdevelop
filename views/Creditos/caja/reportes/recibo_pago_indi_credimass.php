...
function reciboCiacreho($pdf, $registro, $hoy, $usuario, $info)
{
    ...
    $pdf->CellFit($ancho_linea2 * 7, $tamanio_linea + 10, 'Documento No. ' . $registro[0][4], 0, 0, 'C', 0, '', 1, 0);
    ...
    $pdf->CellFit($ancho_linea2, $tamanio_linea, Utf8::decode($format_monto->toMoney($decimal[0])) . $res . "/100", 0, 0, 'L', 0, '', 1, 0);

    $pdf->CellFit($tabular + 20, $tamanio_linea, 'AMORTIZACION A CREDITO / PAGO DE INTERES DE CREDITO DE', 0, 0, 'L', 0, '', 1, 0);
    $pdf->Ln(4);

    //CLIENTE
    $pdf->SetFont($fuente, 'B', 11);
    $pdf->CellFit($ancho_linea2 + 30, $tamanio_linea + 2, " ", 0, 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($tabular + 52, $tamanio_linea + 2, (mb_convert_encoding(mb_strtoupper($registro[0][2], 'utf-8'), 'ISO-8859-1', 'UTF-8')), 0, 0, 'L', 0, '', 1, 0);
    $pdf->Ln(4);
    //DPI 
    $pdf->CellFit($ancho_linea2 + 30, $tamanio_linea + 2, " ", 0, 0, 'L', 0, '', 1, 0);
    ...
    $pdf->CellFit($ancho_linea2 * 2, $tamanio_linea + 2, 'USUARIO: ' . mb_convert_encoding(utf8_decode(mb_strtoupper($usuario, 'utf-8')) , 'ISO-8859-1', 'UTF-8'), 0, 0, 'L', 0, '', 1, 0);

    $pdf->Ln(15);
    $pdf->CellFit($ancho_linea2 * 2, $tamanio_linea + 2, "  ", 0, 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 * 3 + 40, $tamanio_linea + 2, "________________________           ________________________", 0, 0, 'L', 0, '', 1, 0);
    $pdf->Ln(4);
    $pdf->CellFit($ancho_linea2 * 2, $tamanio_linea + 2, "  ", 0, 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 * 3 + 40, $tamanio_linea + 2, "                     CLIENTE                                    " . Utf8::decode(Utf8::decode(mb_strtoupper($usuario, 'utf-8'))), 0, 0, 'L', 0, '', 1, 0);
}
...
function recibo_credireforma($pdf, $registro, $hoy, $usuario, $info)
{
    ...
    $pdf->CellFit($ancho_linea2, $tamanio_linea, trim($registro[0][4]), 0, 0, 'L', 0, '', 1, 0);
