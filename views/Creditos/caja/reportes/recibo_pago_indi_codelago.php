...
function recibo($pdf, $registro, $hoy, $usuario, $info)
{
    ...
    $ancho_linea2 = 30;
    $pdf->ln(17.5);
    // Mover en eje X antes de imprimir las celdas
    $pdf->SetX(55); // Puedes ajustar el valor 15 según lo que necesites
    $pdf->SetFont($fuente, 'B', 7);
    $pdf->CellFit($ancho_linea2, $tamanio_linea - 1, 'Numero de Pago:  ' . $registro[0]['numoperacion'], 0, 0, 'L', 0, '', 1, 0);
    ...
    $pdf->CellFit($ancho_linea2, $tamanio_linea - 1 , mb_strtoupper($registro[0]['fuente'], 'utf-8'), 0, 0, 'L', 0, '', 1, 0); //tipo de fondo
    ...
    $pdf->SetX(55); // Puedes ajustar el valor 15 según lo que necesites
    $pdf->CellFit($ancho_linea2 - 29, $tamanio_linea - 1, "  ", 0, 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2, $tamanio_linea - 1, 'FECHA: ' . setdatefrench($registro[0]['fechadoc']), 0, 0, 'L', 0, '', 1, 0);

    $pdf->CellFit($ancho_linea2 - 5, $tamanio_linea - 1, 'CAPITAL', 'L-R', 0, 'L', 0, '', 1, 0);
    ...
    $pdf->SetFont($fuente, 'B', 7);
    $pdf->CellFit($ancho_linea2, $tamanio_linea - 1, 'CUENTA: ', 0, 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 - 5, $tamanio_linea - 1, 'INTERESES', 'L-R', 0, 'L', 0, '', 1, 0);
    $pdf->SetFont($fuente, '', 7);
    $pdf->CellFit(5, $tamanio_linea - 1, 'Q', 'L', 0, 'C', 0, '', 1, 0);
    ...
    $pdf->CellFit($ancho_linea2 - 29, $tamanio_linea - 1, "  ", 0, 0, 'L', 0, '', 1, 0); //celda vacia 
    $pdf->SetFont($fuente, 'B', 7);
    $pdf->CellFit($ancho_linea2, $tamanio_linea - 1, $registro[0]['ccodcta'], 0, 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 - 5, $tamanio_linea - 1, 'MORA', 'L-R', 0, 'L', 0, '', 1, 0);
    $pdf->SetFont($fuente, '', 7);
    $pdf->CellFit(5, $tamanio_linea - 1, 'Q', 'L', 0, 'C', 0, '', 1, 0);
    ...
    if ($registro[0]['otros'] < 1) {
        ...
        $pdf->CellFit(20, $tamanio_linea - 1, 'SALDO', 'L-R-T', 0, 'L', 0, '', 1, 0);
    }
    $pdf->Ln(3.5);
    ...
    if ($registro[0]['otros'] > 0) {
        ...
        $pdf->CellFit(1, $tamanio_linea - 1, ' ', 0, 0, 'L', 0, '', 1, 0);
        $pdf->SetFont($fuente, 'B', 7);
        $pdf->CellFit(20, $tamanio_linea - 1, 'SALDO', 'L-R-T', 0, 'C', 0, '', 1, 0);
        $pdf->Ln(3.5);
    }

    //*****
    $pdf->SetX(55); // Puedes ajustar el valor 15 según lo que necesites
    $pdf->CellFit($ancho_linea2 + 1, $tamanio_linea - 1, ' ', 0, 0, 'L', 0, '', 1, 0);
    $pdf->SetFont($fuente, 'B', 7);
    ...
    $pdf->CellFit(5, $tamanio_linea - 1, 'Q', 'L-B', 0, 'C', 0, '', 1, 0);
    $pdf->SetFont($fuente, '', 7);
    $pdf->CellFit(15, $tamanio_linea - 1, $registro[0]['saldo'], 'R-B', 0, 'R', 0, '', 1, 0);
    $pdf->Ln(5);
    ...
    //TOTAL EN LETRAS
    $pdf->SetX(55); // Puedes ajustar el valor 40 según lo que necesites
    $pdf->SetFont($fuente, '', 7);
    $decimal = explode(".", $registro[0]['total']);
    $res = ($decimal[1] == 0) ? 0 : $decimal[1];
    ...
    // Usuario
    // $pdf->SetX(50); // Puedes ajustar el valor 40 según lo que necesites
    // $pdf->SetFont($fuente, 'B', 7);
