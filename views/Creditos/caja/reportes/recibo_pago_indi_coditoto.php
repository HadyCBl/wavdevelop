...
function recibo($pdf, $registro, $hoy, $usuario, $info)
{
    ...
    // $pdf->CellFit($ancho_linea2 * 7 + 50, $tamanio_linea-25, $registro[0]['numdoc'], 0, 0, 'C', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 * 7 + 50, $tamanio_linea-25, ' ', 0, 0, 'C', 0, '', 1, 0);
    $pdf->Ln(0);
    
    $pdf->SetFont($fuente, 'B', 11);
    // $pdf->CellFit($ancho_linea2 * 2, $tamanio_linea + 2, "  ", 0, 0, 'L', 0, '', 1, 0);
    $nombre = decode_utf8($registro[0]["nombre"]);
    $pdf->CellFit($ancho_linea2 * 4 + 16, $tamanio_linea + 2, 'NOMBRE: ' . $nombre, 0, 0, 'R', 0, '', 1, 0);
    ...
    $pdf->CellFit(0, $tamanio_linea, "CANTIDAD EN LETRAS: " . decode_utf8($format_monto->toMoney($decimal[0])) . $res . "/100", 0, 1, 'C', 0, '', 1, 0);
    
    //DETALLE
    $detalle = decode_utf8($registro[0]["concepto"]);
    $pdf->CellFit($ancho_linea2 + 25, $tamanio_linea, " ", 0, 0, 'L', 0, '', 1, 0);
    $pdf->MultiCell(120, $tamanio_linea + 1, "DETALLE: " . $detalle, 0, 'L');
    $pdf->Ln(1);

    //fecha
    // $pdf->SetFont($fuente, 'B', 11);
    // $pdf->CellFit($ancho_linea2, $tamanio_linea + 2, " ", 0, 0, 'L', 0, '', 1, 0);
    // $pdf->CellFit($tabular + 52, $tamanio_linea + 2, (setdatefrench($registro[0]['fechaaplica'])), 0, 0, 'L', 0, '', 1, 0);
    // $pdf->Ln(4);
    ...
    $pdf->CellFit($ancho_linea2 * 3, $tamanio_linea, "  ", 0, 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 * 3, $tamanio_linea , 'USUARIO: ' . decode_utf8(decode_utf8(mb_strtoupper($usuario, 'utf-8'))), 0, 0, 'R', 0, '', 1, 0);
}
