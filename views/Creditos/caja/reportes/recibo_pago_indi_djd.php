...
function recibo($pdf, $registro, $hoy, $usuario, $info)
{
    ...
    $pdf->CellFit($ancho_linea2 * 4 - 5, $tamanio_linea - 1, 'Fecha Operacion:  ' . (setdatefrench($registro[0]['fechaaplica'])), 0, 0, 'C', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2  + 10, $tamanio_linea - 1, 'Documento No. ' . $registro[0]['numdoc'], 0, 0, 'C', 0, '', 1, 0);
    ...
    //Tipo de fondo
    $pdf->CellFit($ancho_linea2 + 7, $tamanio_linea - 1, "  ", 0, 0, 'L', 0, '', 1, 0);
    ...
    //No. de cuenta **********
    $pdf->CellFit($ancho_linea2 + 7, $tamanio_linea - 1, "  ", 0, 0, 'L', 0, '', 1, 0); //celda vacia 
    $pdf->SetFont($fuente, 'B', 9);
    $pdf->CellFit($ancho_linea2 + 4, $tamanio_linea - 1, 'CUENTA: ', 0, 0, 'L', 0, '', 1, 0);
    ...
    if ($registro[0]['otros'] > 0) {
        $pdf->CellFit($ancho_linea2 + 41, $tamanio_linea - 1, "  ", 0, 0, 'L', 0, '', 1, 0); //celda bacia 
        $pdf->SetFont($fuente, 'B', 9);
    ...
    //*****
    $pdf->CellFit($ancho_linea2 + 41, $tamanio_linea - 1, ' ', 0, 0, 'L', 0, '', 1, 0);
    $pdf->SetFont($fuente, 'B', 9);
    ...
    if ($registro[0]['otros'] > 0) {
        ...
        $pdf->Ln(2);
        $pdf->CellFit($ancho_linea2 * 3 + 4, $tamanio_linea - 1, "  ", 0, 0, 'L', 0, '', 1, 0); //celda vacía 
        $pdf->Ln(2);
    }
    ...
    $pdf->CellFit($ancho_linea2 + 7, $tamanio_linea + 1, " ", 0, 0, 'L', 0, '', 1, 0); //celda vacía 

    $pdf->CellFit(0, $tamanio_linea - 2 , decode_utf8($format_monto->toMoney($decimal[0])) . $res . "/100", 0, 1, 'L', 0, '', 1, 0);

    $pdf->CellFit($ancho_linea2 + 7, $tamanio_linea - 2, " ", 0, 0, 'L', 0, '', 1, 0);
    $pdf->MultiCell(0 , $tamanio_linea - 2, decode_utf8($registro[0]["concepto"]), 0, 'L');
    $pdf->Ln(1);

    //CLIENTE
    $pdf->SetFont($fuente, 'B', 7);
    // $pdf->CellFit($ancho_linea2 + 25, $tamanio_linea + 1, " ", 0, 0, 'L', 0, '', 1, 0);
    // $pdf->CellFit($tabular + 52, $tamanio_linea + 1, (setdatefrench($registro[0]['fechaaplica'])), 0, 0, 'L', 0, '', 1, 0);
    // $pdf->Ln(4);
    
    //DPI 
    $pdf->CellFit($ancho_linea2 * 4 + 10, $tamanio_linea - 4, " ", 0, 0, 'L', 0, '', 1, 0);
    // $pdf->CellFit($tabular + 33, $tamanio_linea + 1, $registro[0]['dpi'], 0, 0, 'L', 0, '', 1, 0);
    // $pdf->Ln(1);
    $pdf->CellFit($ancho_linea2 * 2, $tamanio_linea - 5 , 'USUARIO: ' . decode_utf8(decode_utf8(mb_strtoupper($usuario, 'utf-8'))), 0, 0, 'L', 0, '', 1, 0);
}