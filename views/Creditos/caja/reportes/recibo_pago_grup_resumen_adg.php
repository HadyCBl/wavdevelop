...
function recibo($pdf, $registro, $hoy, $usuario, $info, $cnuming)
{
    ...
    $pdf->CellFit($ancho_linea2 + 125, $tamanio_linea, ' ', 0, 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2, $tamanio_linea - 10, $hoy, 0, 0, 'L', 0, '', 1, 0);
    $pdf->Ln(40);
    $pdf->SetFont($fuente, 'B', 7);
    //NUMERO DE DOCUMENTO
    $pdf->SetX($pdf->GetX() + $margen);
    // $pdf->CellFit($ancho_linea2 + 70, $tamanio_linea + 1, ' ', 0, 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 - 10, $tamanio_linea, 'Documento No.', 0, 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2, $tamanio_linea, $cnuming, 0, 0, 'L', 0, '', 1, 0);
    $pdf->Ln(4);

    //FECHA DOCTO Y FUENTES
    $pdf->SetX($pdf->GetX() + $margen);
    $pdf->CellFit($ancho_linea2 - 5, $tamanio_linea, 'FECHA DOCTO.', 0, 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 + 20, $tamanio_linea, date("d-m-Y", strtotime($registro[0]['DFECPRO'])), 0, 0, 'L', 0, '', 1, 0);
    $pdf->SetFont($fuente, 'B', 9);
    $pdf->CellFit($ancho_linea2 + 10, $tamanio_linea, 'CONCEPTO', 0, 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 + 5, $tamanio_linea, 'CANTIDAD', 0, 0, 'R', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2, $tamanio_linea, ' ', 0, 0, 'L', 0, '', 1, 0);
    $pdf->Ln(3);

    $pdf->SetFont($fuente, 'B', 7);
    $pdf->SetX($pdf->GetX() + $margen);
    $pdf->CellFit($ancho_linea2 - 5, $tamanio_linea, 'FECHA APLICA:', 0, 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 + 20, $tamanio_linea, date("d-m-Y", strtotime($registro[0]['DFECPRO'])), 0, 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 + 10, $tamanio_linea, 'CAPITAL', 0, 0, 'L', 0, '', 1, 0);
    $pdf->CellFit(5, $tamanio_linea, 'Q', 0, 0, 'C', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2, $tamanio_linea, $sum_cappag, 0, 0, 'R', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2, $tamanio_linea, ' ', 0, 0, 'L', 0, '', 1, 0);
    $pdf->Ln(3);
    ...
    $pdf->SetX($pdf->GetX() + $margen);
    $pdf->CellFit($ancho_linea2, $tamanio_linea, 'NOMBRE DEL GRUPO:', 0, 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 + 15, $tamanio_linea, ' ', 0, 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 + 10, $tamanio_linea, 'INTERESES', 0, 0, 'L', 0, '', 1, 0);
    $pdf->CellFit(5, $tamanio_linea, 'Q', 0, 0, 'C', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2, $tamanio_linea, $sum_intpag, 0, 0, 'R', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2, $tamanio_linea, ' ', 0, 0, 'L', 0, '', 1, 0);
    $pdf->Ln(3);

    //NOMBRE Y MORA
    $pdf->SetX($pdf->GetX() + $margen);
    $pdf->CellFit($ancho_linea2, $tamanio_linea, utf8_decode(mb_strtoupper($registro[0]['NombreGrupo'], 'utf-8')), 0, 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 + 15, $tamanio_linea, ' ', 0, 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 + 10, $tamanio_linea, 'MORA', 0, 0, 'L', 0, '', 1, 0);
    $pdf->CellFit(5, $tamanio_linea, 'Q', 0, 0, 'C', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2, $tamanio_linea, $sum_morpag, 0, 0, 'R', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2, $tamanio_linea, ' ', 0, 0, 'L', 0, '', 1, 0);
    $pdf->Ln(3);

    //PRESTAMO Y OTROS
    $pdf->SetX($pdf->GetX() + $margen);
    $pdf->CellFit($ancho_linea2 - 15, $tamanio_linea, 'CICLO:', 0, 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 * 2, $tamanio_linea, $registro[0]['NCiclo'], 0, 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 + 10, $tamanio_linea, 'OTROS', 0, 0, 'L', 0, '', 1, 0);
    $pdf->CellFit(5, $tamanio_linea, 'Q', 0, 0, 'C', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2, $tamanio_linea, $sum_otro, 0, 0, 'R', 0, '', 1, 0);
    $pdf->SetFont($fuente, 'B', 9);
    $pdf->CellFit($ancho_linea2 - 10, $tamanio_linea, 'SALDO', 0, 0, 'C', 0, '', 1, 0);
    $pdf->SetFont($fuente, 'B', 7);
    $pdf->Ln(3);

    //CANTIDAD EN LETRAS
    $pdf->SetX($pdf->GetX() + $margen);
    $pdf->CellFit($ancho_linea2 + 65, $tamanio_linea, 'Cantidad en letras:', 0, 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 - 10, $tamanio_linea, 'TOTAL', 0, 0, 'C', 0, '', 1, 0);
    $pdf->CellFit(5, $tamanio_linea, 'Q', 0, 0, 'C', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2, $tamanio_linea, $sum_total, 0, 0, 'R', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 - 25, $tamanio_linea, 'Q', 0, 0, 'C', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 - 20, $tamanio_linea, $saldo, 0, 0, 'R', 0, '', 1, 0);
    $pdf->Ln(3);
    ...
    $pdf->MultiCell(0, $tamanio_linea, utf8_decode($format_monto->toMoney($decimal[0], 2, '', '')) . $res . "/100", 0, 'L');
    $pdf->Ln(3);

    $pdf->SetX($pdf->GetX() + $margen);
    $pdf->MultiCell(0, $tamanio_linea, utf8_decode("PAGO DE CRÉDITO A NOMBRE DE " . strtoupper($registro[0]['NombreGrupo']) . " NUMERO DE RECIBO " . $cnuming), 0, 'L');
    $pdf->Ln(20);

    // FIRMAS
    $pdf->SetX($pdf->GetX() + $margen);
    $pdf->CellFit($ancho_linea2 - 20, $tamanio_linea + 1, ' ', 0, 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 + 30, $tamanio_linea + 1, 'F. REPRESENTANTE DEL GRUPO', 'T', 0, 'C', 0, ' ', 1, 0);
    $pdf->CellFit($ancho_linea2, $tamanio_linea + 1, ' ', 0, 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 + 30, $tamanio_linea + 1, 'F. ' . utf8_decode(utf8_decode($usuario)), 'T', 0, 'C', 0, ' ', 1, 0);
    $pdf->Ln(3);
    $pdf->CellFit($ancho_linea2 - 10, $tamanio_linea + 1, ' ', 0, 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 + 30, $tamanio_linea + 1, strtoupper($registro[0]['NombreGrupo']), 0, 0, 'C', 0, ' ', 1, 0);
}
