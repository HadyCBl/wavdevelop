...
function recibo($pdf, $registro, $hoy, $usuario, $info, $resultdetalle, $mover_x = 10)
{
    ...
    //NUMERO DE DOCUMENTO
    $pdf->SetX($pdf->GetX() + $mover_x);
    $pdf->CellFit($ancho_linea2 - 5, $tamanio_linea, 'Numero de pago: ', 0, 0, 'L', 0, ' ', 1, 0);
    $pdf->SetFont($fuente, 'B', 9);
    ...
    //NOMBRE Y MORA
    $pdf->SetX($pdf->GetX() + $mover_x);
    $pdf->CellFit($ancho_linea2 + 20, $tamanio_linea, decode_utf8(strtoupper($registro[0]["nombre"])), 0, 0, 'L', 0, '', 1, 0);
    ...
    // $pdf->SetX($pdf->GetX() + $mover_x);
    // $direccion = isset($registro[0]["direc"]) && !empty($registro[0]["direc"]) ? decode_utf8($registro[0]["direc"]) : decode_utf8('-');
    // $pdf->CellFit($ancho_linea2 * 2.8, $tamanio_linea, $direccion, 0, 0, 'L', 0, ' ', 1, 0);
    // $pdf->Ln(4);
    ...
    // FIRMAS
    // $pdf->SetX($pdf->GetX() + $mover_x);
    // $pdf->CellFit($ancho_linea2 + 53, $tamanio_linea + 1, 'F. ' . decode_utf8(strtoupper($registro[0]["nombre"])), 'T', 0, 'C', 0, ' ', 1, 0);
    // $pdf->CellFit($ancho_linea2 - 20, $tamanio_linea + 1, ' ', 0, 0, 'L', 0, '', 1, 0);
    // $pdf->CellFit($ancho_linea2 + 53, $tamanio_linea + 1, 'F. ' . decode_utf8($usuario), 'T', 0, 'C', 0, ' ', 1, 0);
}
