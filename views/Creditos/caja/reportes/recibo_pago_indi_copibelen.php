...
function recibo($pdf, $registro, $hoy, $usuario, $info)
{
    ...
    $pdf->CellFit($ancho_linea2 + 15, $tamanio_linea + 1, mb_strtoupper($registro[0][6], 'utf-8'), 0, 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2, $tamanio_linea + 1, 'Documento No.', 0, 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 + 10, $tamanio_linea + 1, $registro[0][4], 0, 0, 'L', 0, '', 1, 0);
    $pdf->SetFont($fuente, '', 11);
    $pdf->Ln(8);

    //FECHA DOCTO Y FUENTES
    $pdf->CellFit($ancho_linea2, $tamanio_linea + 1, ' ', 0, 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2, $tamanio_linea + 1, 'FECHA D.', 0, 0, 'R', 0, '', 1, 0);
    ...
    $pdf->SetFont($fuente, '', 11);
    $pdf->CellFit($ancho_linea2, $tamanio_linea + 1, ' ', 0, 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2, $tamanio_linea + 1, 'FECHA A:', 0, 0, 'R', 0, '', 1, 0);
    $pdf->SetFont($fuente, '', 10);
    ...
    $pdf->CellFit($ancho_linea2, $tamanio_linea + 1, ' ', 0, 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 - 2, $tamanio_linea + 1, 'NOMBRE:', 0, 0, 'R', 0, '', 1, 0);
    ...
    $pdf->MultiCell(0, $tamanio_linea + 1, mb_convert_encoding($format_monto->toMoney($decimal[0], 2, '', '') . $res . "/100", 'ISO-8859-1', 'UTF-8'), 0, 'L');

    //PAGO DE CRÉDITO A NOMBRE DE 
    $pdf->Ln(1);
