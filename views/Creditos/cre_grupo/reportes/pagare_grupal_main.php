...
// Imprimir firmas al finalizar la primera página
firmas($pdf, $registro);
...
// Imprimir firmas al finalizar la segunda página
firmas($pdf, $registro);
...
{
    $fpdf->Image('../../../..'.$info[0]["log_img"], 160, 8, 40, 0);
...
function firmas($fpdf, $registro)
{
    ...
    // Calcular cuánto espacio se necesita para todas las firmas
    $num_firmas = count($registro);
    $firmas_por_fila = 4; // 4 firmas por fila (200/50)
    $filas_necesarias = ceil($num_firmas / $firmas_por_fila);
    $altura_necesaria = 15 + ($filas_necesarias * 18); // 15 de salto inicial + 18 por cada fila
    
    // Obtener posición actual y altura disponible de la página
    $y_actual = $fpdf->GetY();
    $margen_inferior = 10; // Margen inferior de seguridad
    $altura_disponible = $fpdf->GetPageHeight() - $margen_inferior - $y_actual;
    
    // Si no cabe, agregar una nueva página
    if ($altura_necesaria > $altura_disponible) {
        $fpdf->AddPage();
    }
    ...
    while ($i < count($registro)) {
        ...
        if ($x_pos + 50 > 200) { // Si no cabe, saltar a nueva línea
            $y_pos += 18; // Aumentar la distancia entre filas de firmas
            $x_pos = 10;
            $fpdf->SetXY($x_pos, $y_pos);
        }
        
        $fpdf->SetXY($x_pos, $y_pos);
        $fpdf->MultiCell(45, 3, strtoupper(decode_utf8($registro[$i]['short_name'])), 'T', 'C', false);
        $x_pos += 50;
        $i++;
    }
}
