...
function printpdf($dataCredito, $info, $fechasPagos, $amortiza, $gastosCuota, $interesShow, $gastosDistribuidos)
{
    class PDF extends FPDF
    {
        ...
        function Header()
        {
            ...
            $this->Cell(70, 5, decode_utf8('No. Cuenta : ') . $this->CCODCTA, 0, 0, 'L');
        ...
        function htable($titulos)
        {
            ...
            $this->SetX(50.5); // para mover el encabezado al centro
        ...
        function btable($planPago, $porCuota)
        {
            ...
            $this->SetFont('Times', '', $sizefont);
            $fill = false;
            $saldointeres = array_sum(array_column($planPago, 'interes'));
            $otrosTotales = 0;
            foreach ($planPago as $row) {
                ...
                $saldointeres = ($saldointeres - $row['interes']);


                $otros = (!empty($row['gastos'])) ? array_sum(array_column($row['gastos'], 'monto')) : 0;
                if ($otros > 0 || $columnOtros) {
                    $otrosTotales += $otros;
                    // $this->Cell($w[5], $higt, moneda($otros), 'LR', 0, 'C', $fill);
                }

                $this->Cell($w[count($w) - 1], $higt, moneda(abs($row['saldo'] + $saldointeres)), 'LR', 0, 'C', $fill);
                $this->Ln();
                $fill = !$fill;
            }
            ...
            // Línea final
            $this->SetX($x);
            $this->Cell($totalWidth, 0, '', 0, 1);

            // Totales
            $this->SetX($x);
            $this->Cell($w[0] + $w[1], $higt, 'TOTAL', 'T', 0, 'C', false);
            $this->Cell($w[2], $higt, moneda(array_sum(array_column($planPago, 'cuota'))), 'T', 0, 'C', false);
            $this->Cell($w[3], $higt, ' ', 'T', 1, 'C', false);
            $this->Ln(4);

            // $this->Cell($w[3], $higt, moneda(array_sum(array_column($planPago, 'capital'))), 0, 0, 'C', false);
            // $this->Cell($w[4], $higt, moneda(array_sum(array_column($planPago, 'interes'))), 0, 0, 'C', false);
            if (!empty($porCuota)) {
                // $this->Cell($w[5], $higt, moneda($otrosTotales), 0, 0, 'C', false);
            }
            $this->Cell(35, $higt, decode_utf8('Número de cuenta a depositar:'), 0, 1, 'L', false);
            $this->Cell(35, $higt, 'IN KUMOOL BUSSINES S.A.: 3442053927', 0, 1, 'L', false);
        }
    }
    ...
    $pdf->AliasNbPages();
    $pdf->AddPage('P', array(216, 330)); // Tamaño oficio personalizado (ancho, alto) en mm

    $pdf->SetFont('Times', '', 12);

    //TITULOS PARA CABECERA
    $titulos = ['Fecha', 'No Cuota', 'Cuota', 'Saldo'];
    if (!empty($gastosCuota)) {
        array_pop($titulos);
        array_push($titulos, 'Saldo');
    }
    $pdf->htable($titulos);
