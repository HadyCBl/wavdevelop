...
$section->addImage(
    '../../../../includes/img/credisuma.png', // Ruta a tu imagen
    [
        'width' => \PhpOffice\PhpWord\Shared\Converter::cmToPixel(4), // Puedes ajustar el tamaño
        'height' => \PhpOffice\PhpWord\Shared\Converter::cmToPixel(2), // Opcional
...
$textRun->addText('En la ciudad de Santa Cruz Barillas, departamento de Huehuetenango, el ' . $fechaenletras . ' (' . $fechadesembolso . '); yo: ' , ['bold' => false, 'name' => 'Arial', 'size' => 9]);
$textRun->addText($nombrecliente , ['bold' => true, 'name' => 'Arial', 'size' => 9]);
$textRun->addText(' de ' . $edadClienteTexto . ' años de edad, guatemalteca, Comerciante, de este domicilio, me identifico con el Documento Personal de Identificación número CUI: ' . $dpiClienteTexto . ' (' . $dpiClienteFormateado . '), extendido por el Registro Nacional de las Personas - RENAP -, en adelante llamado DEUDOR, Actúo en nombre propio y; RECONOZCO QUE DEBO Y PROMETO INCONDICIONALMENTE PAGAR a la orden de ', ['bold' => false, 'name' => 'Arial', 'size' => 9]);
$textRun->addText('CREDISUMA R L, ', ['bold' => true, 'name' => 'Arial', 'size' => 9]);
$textRun->addText('en adelante llamado ACREEDOR, cuyo domicilio y lugar donde se realizará el pago es: 3ra. Ave. 6-06 Zona 1, Municipio de Santa Cruz Barillas, departamento de Huehuetenango, la suma de ' . $monto2 . ' (Q. ' . $monto . '). La suma adeuda deberá cancelarse de la siguiente forma: ' . $cuotaTexto . ' cuotas contínuas, ininterrumpidas de ' . $primeracuota2 . ' (Q. ' . $primeracouta . ') por mes hasta el cumplimiento del crédito. INTERESES: La presente obligación devengara interes a razón del ' . $tasainteres . ' por ciento (' . $tasaintere . '%) anual. FECHA DE VENCIMIENTO: La fecha de vencimiento del presente título de crédito es el ' . $fechavenceletras . ' (' .  $fechavence . '); EFECTOS PROCESALES: ', ['bold' => false, 'name' => 'Arial', 'size' => 9]);
$textRun->addText('a) ' , ['bold' => true, 'name' => 'Arial', 'size' => 9]);
$textRun->addText('Reconozco como título ejecutivo perfecto el presente título de crédito; ' , ['bold' => false, 'name' => 'Arial', 'size' => 9]);
$textRun->addText('b) ' , ['bold' => true, 'name' => 'Arial', 'size' => 9]);
$textRun->addText('Cualquier controversia derivada del cumplimiento del presente título de crédito, seran competentes los tribunales del departamento de huehuetenango, renunciando para el efecto de cualquier otro domicilio que pudiere corresponderme; ' , ['bold' => false, 'name' => 'Arial', 'size' => 9]);
$textRun->addText('c) ' , ['bold' => true, 'name' => 'Arial', 'size' => 9]);
$textRun->addText('Acepto desde ya como buenas y exactas las cuentas que el ACREEDOR, presente acerca de este pagaré y como líquido, exigible y de plazo vencido el saldo que se demande, como consecuencia de esta obligación; ' , ['bold' => false, 'name' => 'Arial', 'size' => 9]);
$textRun->addText('d) ' , ['bold' => true, 'name' => 'Arial', 'size' => 9]);
$textRun->addText('Que el incumplimiento de una sola de las cuotas en los plazos establecidos, da derecho al ACREEDOR a dar por vencida en forma anticipada la presente obligación y a exigir judicialmente el cumplimiento de la misma ' , ['bold' => false, 'name' => 'Arial', 'size' => 9]);
$textRun->addText('e) ' , ['bold' => true, 'name' => 'Arial', 'size' => 9]);
$textRun->addText('Señalo como lugar para recibir notificaciones en ' . $diredomicilio . ', ' . $municipiocli . ', ' . $departamentocli , ['bold' => false, 'name' => 'Arial', 'size' => 9]);
$textRun->addText(' f) ' , ['bold' => true, 'name' => 'Arial', 'size' => 9]);
$textRun->addText('El DEUDOR se obliga a comunicar por escrito cualquier cambio que de ella hiciere, en el entendido que a falta de tal aviso, se tendrán por válidas y bien hechas las citaciones, notificaciones y emplazamientos que se efectúen en el lugar señalado. ' , ['bold' => false, 'name' => 'Arial', 'size' => 9]);
$textRun->addText('g) ' , ['bold' => true, 'name' => 'Arial', 'size' => 9]);
$textRun->addText('CESIÓN: Este pagaré es cedible, negociable mediante simple endoso del acreedor, sin necesidad de consentimiento previo ni posterior notificación al deudor.' , ['bold' => false, 'name' => 'Arial', 'size' => 9]);
...
$cell1->addText('En fe de lo cual firmo y estampo mi huella en este pagaré.', ['name' => 'Arial', 'size' => 9], ['alignment' => Jc::LEFT]);
$cell1->addText('', null, ['spaceAfter' => 0]); // pequeño espaciado
$cell1->addText('', null, ['spaceAfter' => 0]);
$cell1->addText('', null, ['spaceAfter' => 0]);
$cell1->addText('', null, ['spaceAfter' => 0]);
...
$cell2->addText('', null, ['spaceAfter' => 0]); // pequeño espaciado
$cell2->addText('', null, ['spaceAfter' => 0]);
$cell2->addText('', null, ['spaceAfter' => 0]);
$cell2->addText('', null, ['spaceAfter' => 0]);
$cell2->addText('', null, ['name' => 'Arial', 'size' => 9], ['alignment' => Jc::LEFT]);
...
$cell2->addText('FIADOR', ['name' => 'Arial', 'size' => 8], ['alignment' => Jc::CENTER]);
