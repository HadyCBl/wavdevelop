...
function printpdf($datos, $garantias, $info, $conexion)
{
    ...
    $pdf->Cell(0, $tamanio_linea, decode_utf8("FICHA DE APROBACIÓN DE CRÉDITO INDIVIDUAL"), 0, 1, 'C');
    $pdf->Cell(0, $tamanio_linea-1, ' ', 'B', 1, 'C');
    ...
    $pdf->SetFont($fuente, 'B', 9);
    $pdf->Cell(0, $tamanio_linea, decode_utf8("DATOS DEL CRÉDITO EN ESTADO DE APROBADO"), 0, 0, 'C');
    $pdf->Ln(5);
    ...
    $pdf->CellFit($ancho_linea - 20, $tamanio_linea, ' ', 0, 0, 'L', 0, '', 1, 0);
    $pdf->SetFont($fuente, 'B', 9);
    $pdf->CellFit($ancho_linea, $tamanio_linea, decode_utf8('Fec. Aprobación:'), 0, 0, 'L', 0, '', 1, 0);
    $pdf->SetFont($fuente, '', 9);
    $pdf->CellFit($ancho_linea + 30, $tamanio_linea, (($datos[0]['fecapro'] == '' || $datos[0]['fecapro'] == null || $datos[0]['fecapro'] == '0000-00-00') ? ' ' : date("d-m-Y", strtotime($datos[0]['fecapro']))), 0, 0, 'L', 0, '', 1, 0);
    $pdf->Ln(5);
    ...
    foreach ($garantias as $key => $garantia) {
        ...
        $pdf->CellFit($ancho_linea * 2, $tamanio_linea, $garantia["nomtipdoc"] ?? " ", 0, 0, 'C', 0, '', 0, 0);
        $pdf->CellFit($ancho_linea * 2.5, $tamanio_linea, ($direcciongarantia == '' || $direcciongarantia == null) ? ' ' : $direcciongarantia, 0, 0, 'C', 0, '', 0, 0);
    ...
    // Firmas
    $pdf->firmas(3, [$datos[0]['nombre'],$datos[0]['analista'],"Gerente de Agencia"],'Courier');
