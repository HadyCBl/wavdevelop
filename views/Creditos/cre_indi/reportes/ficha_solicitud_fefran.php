...
$strquery = "SELECT cl.idcod_cliente AS id, cl.idcod_cliente AS codcli, cl.no_identifica AS dpi, cl.no_tributaria AS nit, cl.short_name AS nombre, cl.date_birth AS fechacumple, cl.estado_civil AS civil,  
    cl.Direccion AS direccion, cm.cuotassolicita,cm.crecimiento,cm.recomendacion,
    ...
    cm.CCODCTA AS ccodcta, cm.MontoSol AS montosol, (SELECT IFNULL(MAX(cm2.NCiclo),0)+1 AS ciclo FROM cremcre_meta cm2 WHERE cm2.CodCli=cm.CodCli AND cm2.TipoEnti='INDI' AND (cm2.Cestado='F' OR cm2.Cestado='G')) AS ciclo,
    dest.DestinoCredito AS destinocred, sect.SectoresEconomicos AS sececonomico, act.Titulo AS acteconomico,
    pd.cod_producto AS codprod, pd.nombre AS nomprod, pd.descripcion AS descprod, pd.tasa_interes AS intprod, pd.monto_maximo AS monprod , ff.descripcion AS fondoprod, 
...
printpdf($data, $garantias, $info);

function printpdf($datos, $garantias, $info)
{
    ...
    $pdf->Cell(0, $tamanio_linea, decode_utf8("FICHA DE SOLICITUD DE CRÉDITO INDIVIDUAL"), 0, 1, 'C');
    ...
    $pdf->Cell(0, $tamanio_linea, ' ', 'B', 1, 'C');
    $pdf->SetFont($fuente, 'B', 9);
    $pdf->Cell(0, $tamanio_linea, decode_utf8("DATOS DEL CRÉDITO SOLICITADO"), 0, 0, 'C');
    $pdf->Ln(6);
    ...
    $pdf->SetFont($fuente, 'B', 9);
    $pdf->CellFit($ancho_linea, $tamanio_linea, decode_utf8('Fec. Solicitud:'), 0, 0, 'L', 0, '', 1, 0);
    $pdf->SetFont($fuente, '', 9);
    $pdf->CellFit($ancho_linea * 3 - 20, $tamanio_linea, (($datos[0]['fecsol'] == '' || $datos[0]['fecsol'] == null || $datos[0]['fecsol'] == '0000-00-00') ? ' ' : date("d-m-Y", strtotime($datos[0]['fecsol']))), 0, 0, 'L', 0, '', 1, 0);
    $pdf->SetFont($fuente, 'B', 9);
    $pdf->CellFit($ancho_linea, $tamanio_linea, decode_utf8('No Cuotas:'), 0, 0, 'L', 0, '', 1, 0);
    $pdf->SetFont($fuente, '', 9);
    $pdf->CellFit($ancho_linea + 30, $tamanio_linea, (($datos[0]['cuotassolicita'] == '' || $datos[0]['cuotassolicita'] == null) ? ' ' : $datos[0]['cuotassolicita']), 0, 0, 'L', 0, '', 1, 0);
    $pdf->Ln(6);

    $pdf->SetFont($fuente, 'B', 9);
    $pdf->CellFit($ancho_linea, $tamanio_linea, decode_utf8('Crecimiento:'), 0, 0, 'L', 0, '', 1, 0);
    $pdf->SetFont($fuente, '', 9);
    $pdf->CellFit($ancho_linea + 30, $tamanio_linea, (($datos[0]['crecimiento'] == '' || $datos[0]['crecimiento'] == null) ? ' ' : $datos[0]['crecimiento']), 0, 0, 'L', 0, '', 1, 0);
    $pdf->Ln(6);
    $pdf->SetFont($fuente, 'B', 9);
    $pdf->CellFit($ancho_linea, $tamanio_linea, decode_utf8('Recomendacion:'), 0, 0, 'L', 0, '', 1, 0);
    $pdf->SetFont($fuente, '', 9);
    $pdf->CellFit($ancho_linea + 30, $tamanio_linea, (($datos[0]['recomendacion'] == '' || $datos[0]['recomendacion'] == null) ? ' ' : $datos[0]['recomendacion']), 0, 0, 'L', 0, '', 1, 0);
    $pdf->Ln(8);
    ...
    foreach ($garantias as $key => $garantia) {
        ...
        $direcciongarantia = ($garantia["idtipgar"] == 1 && $garantia["idtipdoc"] == 1) ? (($garantia['direccioncli'] == "") ? " " : $garantia['direccioncli']) : $garantia["direccion"];
        $descripciongarantia = ($garantia["idtipgar"] == 1 && $garantia["idtipdoc"] == 1) ? "NOMBRE: " . $garantia['nomcli'] : "DESCRIPCION: " . $garantia["descripcion"];
        ...
        $pdf->CellFit($ancho_linea * 2.5, $tamanio_linea, (trim($direcciongarantia) !== "" ? $direcciongarantia : " "), 0, 0, 'C', $garantia['marcado'], '', 0, 0);
    ...
    $pdf->SetFont($fuente, 'B', 9);
    $pdf->CellFit($ancho_linea - 25, $tamanio_linea, 'F.', 'B', 0, 'L', 0, '', 1, 0);
    $pdf->SetFont($fuente, '', 9);
    ...
    $pdf->SetFont($fuente, 'B', 9);
    $pdf->CellFit($ancho_linea + 60, $tamanio_linea, (($datos[0]['nombre'] == '' || $datos[0]['nombre'] == null) ? ' ' :  strtoupper(decode_utf8($datos[0]['nombre']))), 0, 0, 'C', 0, '', 1, 0);
    $pdf->SetFont($fuente, '', 9);
    $pdf->CellFit($ancho_linea - 20, $tamanio_linea, ' ', 0, 0, 'L', 0, '', 1, 0);
    $pdf->SetFont($fuente, 'B', 9);
    $pdf->CellFit($ancho_linea + 60, $tamanio_linea, (($datos[0]['analista'] == '' || $datos[0]['analista'] == null) ? ' ' : strtoupper(decode_utf8($datos[0]['analista']))), 0, 0, 'C', 0, '', 1, 0);
    $pdf->SetFont($fuente, '', 9);
    $pdf->Ln(8);

    $pdf->SetFont($fuente, '', 9);
    $pdf->CellFit($ancho_linea + 60, $tamanio_linea, 'Cliente', 0, 0, 'C', 0, '', 1, 0);
    $pdf->SetFont($fuente, '', 9);
    $pdf->CellFit($ancho_linea - 20, $tamanio_linea, ' ', 0, 0, 'C', 0, '', 1, 0);
    $pdf->SetFont($fuente, '', 9);
    $pdf->CellFit($ancho_linea + 60, $tamanio_linea, 'Analista', 0, 0, 'C', 0, '', 1, 0);
    $pdf->SetFont($fuente, '', 9);
    $pdf->Ln(6);
    ...
    $opResult = array(
        ...
        'namefile' => "FichaSolicitud-" . (($datos[0]['ccodcta'] == '' || $datos[0]['ccodcta'] == null) ? ' ' : ($datos[0]['ccodcta'])),
        'tipo' => "pdf",
