...
function printpdf($codigo, $conexion, $idagencia, $db_name_general)
{
    ...
    $pdf->Cell($ancho_linea2 * 2 - 5, $tamanio_linea, " ", 0, 0, 'L');
    $pdf->Cell($ancho_linea2 + 10, $tamanio_linea, "CLIENTE: " . $registro[0]["idcod_cliente"], 0, 0, 'L');
    $pdf->Cell(0, $tamanio_linea, "NOMBRE:" . utf8_decode(mb_strtoupper($registro[0]["short_name"], 'utf-8')), 0, 1, 'L');

    $fecha = date("d-m-Y", strtotime($registro[0]["DFECPRO"]));
    $pdf->Cell($ancho_linea2 * 2 - 5, $tamanio_linea, " ", 0, 0, 'L');
    $pdf->Cell($ancho_linea2 * 2, $tamanio_linea, "FECHA DE EMISION: " . $fecha, 0, 0, 'L');

    $entidadnombre = ($registro[0]['TipoEnti'] == "GRUP") ? 'GRUPO: ' . $registro[0]['NombreGrupo'] : 'CREDITO INDIVIDUAL';
    $pdf->CellFit(0, $tamanio_linea, $entidadnombre, 0, 1, 'C', 0, '', 1, 0);

    // $pdf->CellFit(0, $tamanio_linea, "DESCRIPCION: " . utf8_decode($registro[0]["CCONCEP"]), 0, 1, 'L', 0, '', 1, 0);
    $pdf->Cell($ancho_linea2 * 2 - 5, $tamanio_linea, " ", 0, 0, 'L');
    $pdf->MultiCell(0, $tamanio_linea, "DESCRIPCION: " . utf8_decode($registro[0]["CCONCEP"]), '', 'L');
    ...
    $pdf->Cell($ancho_linea2 + 20, $tamanio_linea, "INTEGRACION DEL PRESTAMO", 1, 0, 'C');
    ...
    $pdf->CellFit($ancho_linea2 + 20, $tamanio_linea, 'CAPITAL', 'RL', 0, 'L', 0, '', 1, 0);
    ...
    while ($gastosflag == true && $fila < count($gastos)) {
        ...
        $pdf->CellFit($ancho_linea2 + 20, $tamanio_linea, $gastos[$fila]["concepto"], 'RL', 0, 'L', 0, '', 1, 0);
    ...
    if ($gastoscredkar > 0 && $fila == 0) {
        $pdf->Cell($ancho_linea2 * 2 - 5, $tamanio_linea, " ", 0, 0, 'L');
        $pdf->CellFit($ancho_linea2 + 10, $tamanio_linea, " ", 'RL', 0, 'C', 0, '', 1, 0);
        $pdf->CellFit($ancho_linea2 + 20, $tamanio_linea, 'DESCUENTOS', 'RL', 0, 'L', 0, '', 1, 0);
        $pdf->CellFit($ancho_linea2 + 10, $tamanio_linea, number_format($gastoscredkar, 2, '.', ','), 'RL', 1, 'R', 0, '', 1, 0);
    }
    $sal = $sal - $gastoscredkar;
    $pdf->SetFont($fuente, 'B', 8);
    $pdf->Cell($ancho_linea2 * 2 - 5, $tamanio_linea, " ", 0, 0, 'L');
    $pdf->CellFit($ancho_linea2 + 10, $tamanio_linea * 2, ' ', 'RL', 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 + 20, $tamanio_linea * 2, 'A ENTREGAR', 'RL', 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 + 10, $tamanio_linea * 2, number_format($sal, 2, '.', ','), 'RL', 1, 'R', 0, '', 1, 0);

    $format_monto = new NumeroALetras();
    $montoletra = $format_monto->toMoney($sal, 2, 'QUETZALES', 'CENTAVOS');
    $pdf->Cell($ancho_linea2 * 2 - 5, $tamanio_linea, " ", 0, 0, 'L');
    $pdf->CellFit($ancho_linea2, $tamanio_linea * 1.5, 'CANTIDAD EN LETRAS: ', 'BTL', 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 * 3 + 10, $tamanio_linea * 1.5, $montoletra, 'BTR', 1, 'L', 0, '', 1, 0);
