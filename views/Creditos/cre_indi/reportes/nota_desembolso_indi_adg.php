...
function printpdf($codigo, $conexion, $idagencia, $db_name_general)
{
    ...
    $pdf->CellFit(0, $tamanio_linea, 'TIPO DE CREDITO: ' . $entidadnombre, 0, 1, 'L', 0, '', 1, 0);
    ...
    $pdf->Cell($ancho_linea2 + 50, $tamanio_linea, Utf8::decode("DESCRIPCION INTEGRACION PRÉSTAMO"), 1, 0, 'C');
    ...
    if (!empty($dataConsulta)) {
        foreach ($dataConsulta as $row) {
            ...
            //datos en la tabla
            $pdf->CellFit($ancho_linea2 + 10, $tamanio_linea, $row['ccodcta'], 'RL', 0, 'C', 0, ' ', 1, 0);
            $pdf->CellFit($ancho_linea2 + 50, $tamanio_linea, Utf8::decode($row['cdescrip']), 'RL', 0, 'L', 0, ' ', 1, 0);
            $pdf->CellFit($ancho_linea2 + 5, $tamanio_linea, number_format($row['debe'], 2, '.', ','), 'RL', 0, 'R', 0, ' ', 1, 0);
            $pdf->CellFit($ancho_linea2 + 5, $tamanio_linea, number_format($row['haber'], 2, '.', ','), 'RL', 1, 'R', 0, ' ', 1, 0);
        }
    } else {
    ...
    $pdf->CellFit($ancho_linea2 + 50, $tamanio_linea * 2, 'TOTALES', 'RL', 0, 'L', 0, ' ', 1, 0);
    $pdf->CellFit($ancho_linea2 + 5, $tamanio_linea * 2, number_format($totalDebe, 2, '.', ','), 'RL', 0, 'R', 0, ' ', 1, 0);
    $pdf->CellFit($ancho_linea2 + 5, $tamanio_linea * 2, number_format($totalHaber, 2, '.', ','), 'RL', 1, 'R', 0, ' ', 1, 0);
    ...
    $pdf->Cell(60, $tamanio_linea, "Elaborado Por Contabilidad", 'T', 0, 'C');
    $pdf->Cell(60, $tamanio_linea, " ", 0, 0, 'C');
    $pdf->Cell(60, $tamanio_linea, "Recibi Conforme", 'T', 0, 'C');
    ...
    $pdf->SetFont($fuente, 'B', 7);
    $pdf->SetX(70);
    $pdf->Cell(60, $tamanio_linea, "Revisado y Autorizado. Jefe de Agencia", 'T', 1, 'C');
