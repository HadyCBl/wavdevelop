...
function printpdf($codigo, $conexion, $idagencia, $db_name_general)
{
    ...
    $pdf->CellFit($ancho_linea2 * 2, $tamanio_linea + 1, 'ELABORADO POR: ' . $_SESSION['nombre'] . ' ' . $_SESSION['apellido'], 'T', 0, 'C', 0, '', 1, 0); // cuenta
    $pdf->CellFit($ancho_linea2, $tamanio_linea + 1, ' ', '', 0, 'R', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 * 2, $tamanio_linea + 1, 'CLIENTE: '. Utf8::decode(mb_strtoupper($registro[0]["short_name"], 'utf-8')), 'T', 1, 'C', 0, '', 1, 0); // cuenta
    // $pdf->Output();
