...
$query = "SELECT cre.DFecDsbls fechades,cre.DFecVen fecven,cre.noPeriodo cuotas,cre.NMonCuo moncuota,cre.Cestado,NCapDes mondes,MonSug monsug,cli.idcod_cliente ccodcli,cli.short_name nombrecli,cli.primer_name,cli.segundo_name,
                ...
                IFNULL((SELECT monto FROM cre_productos_gastos WHERE id_producto=cre.CCODPRD ORDER BY CASE WHEN distribucion IS NOT NULL AND distribucion != '' THEN 0 ELSE 1 END LIMIT 1),0) monto_producto
...
$qdatosppg = "SELECT ppg.ccodcta, ppg.nintere interesesppg, ppg.ncapita capitalppg, ppg.cnrocuo cuotasppg, ppg.dfecven fechavenppg, ppg.OtrosPagos otrosppg,ppg.SaldoCapital saldo_capital, ppg.NAhoProgra vinculado,
                ...
                WHERE ccodcta=?
                ORDER BY cnrocuo;";
$porcentmora = "SELECT porcentaje_mora 
                FROM cre_productos 
                INNER JOIN cremcre_meta ON cre_productos.id=cremcre_meta.CCODPRD 
                WHERE cremcre_meta.CCODCTA=?;";
$qgarantia = "SELECT clig.descripcionGarantia AS descripcionGarantia
                FROM cli_garantia clig
                INNER JOIN cremcre_meta cre ON cre.codcli=clig.idCliente
                INNER JOIN tb_garantias_creditos tgc ON tgc.id_cremcre_meta=cre.CCODCTA
                WHERE cre.CCODCTA=?
                ORDER BY clig.idTipoDoc  DESC LIMIT 1;";
...
try {
    ...
    $porcentmora_result = $database->getAllResults($porcentmora, [$codcuenta]);
    if (empty($porcentmora_result)) {
        $showmensaje = true;
        throw new Exception("No se encontró el porcentaje de mora");
    }
...
$tasaintereLetra = numeroALetras($tasaintere);
...
$monto_gasto_producto = $result[0]["monto_producto"];
$monto_gasto_producto_letras = numeroALetras($monto_gasto_producto);
...
$monto_letras = $format_monto->toMoney($monto, 2, 'QUETZALES', 'CENTAVOS');
...
$mora_porcentaje = $porcentmora_result[0]["porcentaje_mora"];
$mora_porcentaje_letras = numeroALetras($mora_porcentaje);
...
$tasaintereLetra = numeroALetras($tasaintere);
$tasaintereLetraModified = substr($tasaintereLetra, 0, strrpos($tasaintereLetra, ' '));

$mora_porcentaje_letras_modified = substr($mora_porcentaje_letras, 0, strrpos($mora_porcentaje_letras, ' '));
$monto_gasto_producto_letras_modified = substr($monto_gasto_producto_letras, 0, strrpos($monto_gasto_producto_letras, ' '));

$textRun = $section->addTextRun($paragraphJustify);
$textRun->addText("En el municipio de Tecpán Guatemala, departamento de Chimaltenango el día " . fechaCompleta($hoy) . ". YO: ". $nombrecliente. " de " . $edadClienteTexto . " años de edad, " . $estadocivilcli . ", " . $profesioncliente . ", guatemalteca de este domicilio, con residencia en " . $diredomicilio . " jurisdicción de este municipio de " . $municipiocli . ", departamento de " . $departamentocli . ", me identifico con el documento personal de identificación código único de identificación números: " . $dpiClienteTexto . ", extendido por el Registro Nacional de las Personas de la República de Guatemala. Por medio del presente Título de Crédito consistente en PAGARÉ prometo incondicionalmente pagar a la orden de la entidad denominada, MICROEMPRENDEDORA GUATEMALTECA, SOCIEDAD ANÓNIMA la que podrá abreviarse MIEMPREGUA, el cual se encuentra debidamente inscrito en el Registro Mercantil General de la República de Guatemala, bajo el número de registro SIETE MIL CUATROCIENTOS SEIS (7406) FOLIO QUINIENTOS OCHENTA Y OCHO (588) DEL LIBRO VEINTIDOS (22) DE AUXILIARES DE COMERCIO, con domicilio en la Aldea Chuachali, del municipio de Tecpán Guatemala, departamento de Chimaltenango, por la cantidad de " . $monto_letras . " (Q " . $monto . ") los que recibo (en efectivo o cheque número) los que serán pagados mediante " . $cuotaTexto . " (" . $nocuotas . ") cuotas o amortizaciones consecutivas, como lo expresa la siguiente tabla", ['bold' => false, 'name' => 'Arial', 'size' => 8]);
...
$textRun = $section->addTextRun($paragraphJustify);
$textRun->addText("Bajo las condiciones siguiente: a) plazo: será de " . $nocuotas . " " . $tiposPeridodoText . " computables a partir de la fecha de emisión del presente pagaré, sin  embargo y sin prejuicio de lo anterior, la falta de pago de una cuota de capital y/o interés, así como el incumplimiento de cualquiera otra obligación que me imponga la ley o el contenido del presente documento dará derecho a la entidad acreedora a dar por vencido anticipadamente el plazo estipulado y exigir el pago del saldo adeudado, por ende, la obligación será de plazo vencido y el saldo de la deuda se reputará el líquido exigible b)interés: La suma adeudada devengará una tasa de interés de " . $tasaintereLetraModified. " por ciento (" . $tasaintere . "%) " . $tipoPeriodoText1 . " sobre saldos, esta tasa es susceptible de ajuste por parte de la entidad acreedora, con el fin de hacerla según las políticas económica del Gobierno de Guatemala, interés que va calculada e incluido en la cuota antes indicada, la tasa el interés es pactada libremente por el deudor c) por el incumplimiento de pago de cada una de las cuotas, se pagará un  recargo por mora del " . $mora_porcentaje_letras_modified . " por ciento (" . $mora_porcentaje . "%) " . $tipoPeriodoText1 . " sobre cuota o cuotas atrasadas computables de " . $tipoPeriodoText3 . " a " . $tipoPeriodoText3 . ", cuyo porcentaje podrá ser variable por decisión unilateral de la acreedora, y en lo que fuere pertinente desde ya el deudor se compromete a pagar los gastos que directa e indirectamente ocasione esta negociación, incluyendo los de cobranza judicial y/o extrajudicial, honorarios de abogados si fuere necesario; d) Comisiones y descuentos: Habiendo quedado obligado a pagar un monto de MIL DOSCIENTOS CINCUENTA  (Q1,250.00) quetzales equivalente al " . $monto_gasto_producto_letras_modified . " por ciento (" . $monto_gasto_producto. "%) por concepto de comisión por manejo de cuenta; e) En caso juicio, ni el tenedor de este pagaré ni los auxiliares que propaga deberán  prestar garantía, si fuera necesario realizar remate servirá de base el avalúo o monto del adeudo la primera postura a opción del tenedor de este pagaré. El presente pagaré se emite libre de protesto y formalidades de presentación cobro o requerimiento. f) CONDICIONES ESPECIALES: Para el cumplimento de la entidad acreedora expresamente el deudor dejo como garantía prendaria el testimonio del documento que se describe. " . $descripcionGarantia . ". Desde ya estoy sabido de las repercusiones legales por mi incumplimiento me comprometo a otorgar la escritura traslativa de dominio en la modalidad jurídica de obligación escriturar sin objeción alguna. Acepto que en caso incumpla las obligaciones de pago en los términos declarados se proceda al embargo judicial de mis bienes, especialmente el(los) descrito(s) en la parte sustancial de este título de crédito expresamente dejo constancia que todos y cada uno de los datos que he proporcionado a la entidad acreedora en los documentos de solicitud de crédito y estado patrimonial son verídicos y exactos, en la  inexactitud o falta de veracidad de los mismos que la acreedora determine deberá tenerse como una acción dolosa de mi parte generadora de prejuicios a su patrimonio susceptible del ejercicios  de acción penal que en derecho corresponda a esta última. CESIÓN: los derechos que incorpora el presente título, así como el propio título, podrán ser endosados, cedidas o negociados en cualquier forma sin necesidad de previa o anterior notificación al emisor. PACTO DE SUMISIÓN: Por incumplimiento de las obligaciones para con la entidad acreedora expresamente el librador y el(los) avalista(s) renuncian al fuero de sus domicilios y se someten a los tribunales q el tenedor de este pagaré elija, señala como lugar para recibir raciones y/o notificaciones el ya descrito aceptando como buenas y validas los que allí se me hagan. Desde ya acepto como buenas, exactas, líquidas, exigibles y de plazo vencidos las cuentas que por la presente obligación se me reclamen y como título ejecutivo el presente documento cuyo contenido declara conocer y entender.", ['bold' => false, 'name' => 'Arial', 'size' => 8]);
...
// === Fila 1: LIBRADOR ===
$table->addRow(1000); // Aumentar altura de la fila
$cellLibrador = $table->addCell(9000);
...
$cellTextL = $subTableL->addCell(6000, ['valign' => 'center']);
...
// Celda del recuadro a la derecha - AJUSTADO
$subTableL->addCell(2000, [
    'borderSize' => 1,
    'borderColor' => '000000',
    'width' => 2000,
    'height' => 4000, // Aumentar altura del recuadro
    'valign' => 'center'
]);

// === Fila 2: Texto extra ===
$table->addRow(1000); // Aumentar altura de la fila
$cellTextoExtra = $table->addCell(9000);
$cellTextoExtra->addTextBreak(4);

$textRunExtra = $cellTextoExtra->addTextRun($paragraphJustify);
$textRunExtra->addText('POR AVAL : Yo.  ' . $nombrecompletoaval . ' de ' . $edadFiadorTexto . ' años de edad, Estado Civil ' . $estadocivilfiador . ', guatemalteco de profesión, ' . $profesionfiador . ', de este domicilio señalo para recibir notificaciones, mi residencia ubicado en ' . $direccionaval . ' Municipio de ' . $municipiocli . ', Departamento ' . $departamentocli . ',  me identifico con el documento personal de identificación código único número ' . $dpiFiadorTexto . ', extendida por el Registro Nacional de las Personas de la República de Guatemala. En forma mancomunada y solidaria me constituyo en aval por la totalidad de la obligación contraída por el deudor principal en las mismas condiciones pactadas.', ['bold' => false, 'name' => 'Arial', 'size' => 8]);

// === Fila 3: AVALISTA ===
$table->addRow(1000); // Aumentar altura de la fila
$cellAvalista = $table->addCell(9000);
$cellAvalista->addTextBreak(2);
...
// Celda de texto centrado
$cellTextR = $subTableR->addCell(6000, ['valign' => 'center']);
