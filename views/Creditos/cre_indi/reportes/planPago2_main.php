...
function printpdf($dataCredito, $info, $fechasPagos, $amortiza, $gastosCuota, $interesShow, $gastosDistribuidos)
{
    class PDF extends FPDF
    {
        ...
        function htable($titulos)
        {
            ...
            $this->SetX($x + 20); // posición inicial centrada
        ...
        function btable($planPago, $porCuota)
        {
            ...
            foreach ($planPago as $row) {
                ...
                $this->SetX($x - 5); // mover cursor para centrar cada fila
            ...
            // Línea final
            $this->SetX($x - 5); // mover cursor para centrar la línea final
            $this->Cell($totalWidth, 0, '', 'T', 1);

            // Totales
            $this->SetX($x - 5);
            $this->Cell($w[0] + $w[1], $higt, 'TOTALES', 0, 0, 'C', false);
            $totalSumacuota = array_sum(array_column($planPago, 'cuota'));
            $this->Cell($w[2], $higt, moneda($totalSumacuota), 0, 0, 'C', false);
    ...
    $titulos = ['Fecha', 'No Cuota', 'Cuota', 'Capital', 'Interes', 'Otros', 'Saldo Cap'];
