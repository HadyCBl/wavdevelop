...
$strquery = "SELECT cli.idcod_cliente,cli.short_name,cli.url_img,cli.tel_no1,cli.date_birth,cli.no_identifica,cli.Direccion,
                cr.CCODCTA,cr.CCODPRD,cr.Cestado,cr.DFecDsbls,cr.ActoEcono,cr.CtipCre,cr.NtipPerC,cr.Cdescre,cr.CSecEco,cr.CCodGrupo,cr.NCiclo,cr.TipDocDes,cr.noPeriodo,cr.MonSug,cr.NCapDes, cr.TipoEnti,cr.NIntApro, cr.cntAho, cr.moduloafecta,
...
function printpdf($registro, $info, $pagos, $haypagos, $garantias, $sumaCuotaSiguiente, $isVencido)
{
    ...
    class PDF extends FPDF
    {
        ...
        function Header()
        {
            ...
            // MUESTRA LA CUENTA VINCULADA SOLO SI LOS CAMPOS NO ESTAN VACIOS O EN CERO
            if (
                $this->datos[0]["moduloafecta"] == 0 |
                $this->datos[0]["cntAho"] == 0 |
                empty($this->datos[0]["moduloafecta"]) |
                empty($this->datos[0]["cntAho"])
            ) {
                $this->Cell($ancho_linea, 5, 'LINEA DE CREDITO: ', '', 0, 'L');
                $this->Cell($ancho_linea * 1.5, 5, $this->datos[0]["nnompro"], '', 1, 'L');
            }else {
                $this->Cell($ancho_linea, 5, 'LINEA DE CREDITO: ', '', 0, 'L');
                $this->Cell($ancho_linea * 1.5, 5, $this->datos[0]["nnompro"], '', 0, 'L');

                if ($this->datos[0]["moduloafecta"] == 1){
                    
                    $this->Cell($ancho_linea, 5, 'CNT. AHORRO VINCULADA: ', '', 0, 'L');
                    $this->Cell($ancho_linea * 1.5, 5, $this->datos[0]["cntAho"], '', 1, 'L');
                }else{ 
                    $this->Cell($ancho_linea, 5, 'CNT. APORTACION VINCULADA: ', '', 0, 'L');
                    $this->Cell($ancho_linea * 1.5, 5, $this->datos[0]["cntAho"], '', 1, 'L');
                }
