...
$GLOBALS['banderafirma'] = 0; // defaut GLOBAL
function establecerFirma($firma_fiador_k) {
    ...
    if ($firma_fiador_k = 1) {
        ...
        $GLOBALS['firma_fiador_texto_0'] = ' dejamos la impresión dactilar de nuestros dedos pulgares derechos donde corresponde';
        $GLOBALS['firma_fiador2_texto'] = ' por no saber firmar dejo la impresión dactilar de mi dedo pulgar derecho firmando, ruego';
...
function printpdf($info, $data, $creppg,$cod_pagare,$shortname_fiador,$dpi_fiador,$departamento_fiador,$municipio_fiador,$firma_fiador, $segundo_fiador,$shortname_fiador2,$dpi_fiador2,$departamento_fiador2 ,$municipio_fiador2 ,$firma_fiador2, $banderafirma )
{
    ...
    $GLOBALS['banderafirma'] = 0; // default GLOBAL
    
    function establecerFirmacli($firma_cli_k) {
        if ($firma_cli_k == 1) { // comparación, no asignación
            $GLOBALS['firma_cli_texto_0'] = ' dejo mi firma al pie de la misma';
        } else if ($firma_cli_k == 0) { // comparación, no asignación
            $GLOBALS['firma_cli_texto_0'] = ' dejo la impresión dactilar de mi dedo pulgar derecho al pie de la misma.';
        }    
    }
    ...
    //PARTE INICIAL
    $texto = limpiar(introduccion($nombrecliente, $dpi, $dpiletras, $edadletra, $depadomicilio, $depaextiende, $munidomicilio, $direccion, $montoletras, $monto, $nocheque, $nombanco, $destinocredito, $referencia , $profesion , $estadocivil, $ccodcta, $cli_firma, $segundo_fiador));
    ...
    $pdf->WriteTag(0, 5, $texto, 0, "J", 0, 0);
         ...
         if ( $cli_firma = 'si' ) {
            ...
            for ($i = 0; $i < 5; $i++) {
                ...
                $pdf->Cell(80, 5, ' ', 0, 0, 'R');
                $pdf->Cell(28, 5, ' ', "$lineaabajo" . "RL$lineaarriba", 0, 'C');
                $pdf->Cell(40, 5, ' ', 0, 0, 'R');
                $pdf->Ln(5);
                $lineaarriba = "";
                $lineaabajo = "";
...
if ($GLOBALS['banderafirma'] = 1) {
    $pdf->firmas(2, [$shortname_fiador,$testigo]);
    $pdf->Ln(5);
} else  { // Usamos "else if" para una estructura más clara
    $lineaarriba = "";
    $lineaabajo = "";
    for ($i = 0; $i < 5; $i++) {
        if ($i == 0) {
...

{
    ...
    referencia: ' . $referencia . '. Por el presente <vb>PAGARÉ</vb> libre de protesto, prometo pagar incondicionalmente a la orden o endoso de <vb>"MultiInversiones del Norte)"</vb> la que podrá abreviarse <vb>"Inversiones del Norte"</vb>, en adelante llamada "Acreedor y/o entidad Acreedora", la suma 
    de ' . $montoletra . ' (Q ' . number_format($monto) . '), los que recibo a través del cheque número ' . $nocheque . ', del Banco ' . $nombanco . ', Sociedad Anónima, de la cual me 
...
function puntoa2($fechaven)
{
    ...
    la entidad acreedora, situada en zona 1 Playa grande, Ixcan en edificio Kratos, Guatemala, 
    departamento de Quiche, de preferencia, y hago constar que el lugar descrito lo conozco plenamente, o a través de depósito Bancario de la 
    entidad acreedora cuenta monetaria número <vb>522-002213-0 </vb> a nombre de MULTIINVERSIONES DEL NORTE, SOCIEDAD,  
    del Banco Industrial, Sociedad Anónima.</p>');
...
function puntosc2($fechadesembolso, $firma_cli_k) { 
    ...
    if (!$ejecutado_puntosc2) {
        ...
        contraídas en este documento en forma distinta a la pactada, cuyo contenido declara conocer y entender, para lo cual signo la presente, 
        de la misma manera '. $GLOBALS['firma_cli_texto_0'] .' </p>');
        $data .= decode_utf8('<p>Lugar y fecha de Emisión:</p>');
        $data .= decode_utf8('<p>Playa grande, Ixcan Quichè, Guatemala,' . $fechadesembolso . ' </p>');
...
function dpiletra($numdpi )
{
    ...
    $letra_dpi1 = numletras($parte1);
    $letra_dpi2 = numletras($parte2);
    $letra_dpi3 = numletras($parte3);
