...
function printpdf($datos, $garantias, $gastos, $info, $flag3, $conexion)
{
    ...
    $texto = "<p><pers><vb>EL COMITE DE CREDITO DE LA " . decode_utf8($info[0]["nomb_comple"]) . "" . $vlrs[1] . ", CON SEDE EN LA CIUDAD DE " . decode_utf8(strtoupper($info[0]["muni_lug"])) . ", GUATEMALA.</vb></pers> Con base al reglamento general de " . decode_utf8('créditos') . " de " . decode_utf8($vlrs[1]) . " y el dictamen No. " . (($datos[0]['dictamen'] == '' || $datos[0]['dictamen'] == null) ? ' ' : decode_utf8($datos[0]['dictamen'])) . " del departamento de " . decode_utf8('créditos') . " con fecha " . $fechahoy . "; el " . decode_utf8('cómite') . " de " . decode_utf8('créditos') . " con la facultad que le confiere.--------</p>";
    ...
    $pdf->CellFit($ancho_linea, $tamanio_linea, decode_utf8('Tip. Garantia'), 'B', 0, 'C', 1, '', 1, 0);
    $pdf->CellFit($ancho_linea + 10, $tamanio_linea, decode_utf8('Tip. Documento'), 'B', 0, 'C', 1, '', 1, 0);
    $pdf->CellFit($ancho_linea + 40, $tamanio_linea, decode_utf8('Descripción'), 'B', 0, 'C', 1, '', 1, 0);
    $pdf->CellFit($ancho_linea + 20, $tamanio_linea, decode_utf8('Dirección'), 'B', 0, 'C', 1, '', 1, 0);
    $pdf->Ln(6);
    ...
    for ($i = 0; $i < count($garantias); $i++) {
        ...
        if ($resaltado == 1) {
            ...
            //tipo documento
            $pdf->MultiCell($ancho_linea + 10, $tamanio_linea, (($garantias[$i]['nomtipdoc'] == '' || $garantias[$i]['nomtipdoc'] == null) ? ' ' : decode_utf8($garantias[$i]['nomtipdoc'])), 0, 'L', $resaltado);
            $x = $x + ($ancho_linea + 10);
            $yaux = $pdf->GetY();
            ...
            if ($garantias[$i]["idtipgar"] == 1 && $garantias[$i]["idtipdoc"] == 1) {
                ...
                $pdf->MultiCell($ancho_linea + 40, $tamanio_linea, (($garantias[$i]['nomcli'] == '' || $garantias[$i]['nomcli'] == null) ? ' ' : decode_utf8($garantias[$i]['nomcli'])), 0, 'L', $resaltado);
                $x = $x + ($ancho_linea + 40);
                $yaux = $pdf->GetY();
                ...
                $pdf->MultiCell($ancho_linea + 20, $tamanio_linea, (($garantias[$i]['direccioncli'] == '' || $garantias[$i]['direccioncli'] == null) ? ' ' : decode_utf8($garantias[$i]['direccioncli'])), 0, 'L', $resaltado);
                $x = $x + ($ancho_linea + 20);
                $yaux = $pdf->GetY();
                ...
                $pdf->MultiCell($ancho_linea + 40, $tamanio_linea, (($garantias[$i]['descripcion'] == '' || $garantias[$i]['descripcion'] == null) ? ' ' : decode_utf8($garantias[$i]['descripcion'])), 0, 'L', $resaltado);
                $x = $x + ($ancho_linea + 40);
                $yaux = $pdf->GetY();
                ...
                $x = $pdf->GetX();
                //direccion cuando
                $pdf->MultiCell($ancho_linea + 20, $tamanio_linea, (($garantias[$i]['direccion'] == '' || $garantias[$i]['direccion'] == null) ? ' ' : decode_utf8($garantias[$i]['direccion'])), 0, 'L', $resaltado);
                $x = $x + ($ancho_linea + 20);
                $yaux = $pdf->GetY();
    ...
    $pdf->SetStyle("p", $fuente, "N", 10, "0,0,0", 0);
    $texto = "<p> CIUDAD DE " . decode_utf8(strtoupper($info[0]["muni_lug"])) . ", GUATEMALA, " . strtoupper($fechahoy) . ".</p>";
    $pdf->WriteTag(0, 5, $texto, 0, "J", 0, 0);
    $pdf->Ln(16);
