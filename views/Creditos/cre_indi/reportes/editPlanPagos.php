...
function printpdf($conexion, $codusu, $usuCli, $codCu, $db_name_general)
{
    ...
    $pdf->Cell(0, $tamanio_linea, "Plan de pago - " . Utf8::decode($res), 0, 1, 'L');
    ...
    $pdf->Cell(0, 0, (strtoupper(Utf8::decode($usuCli))), 0, 0, 'L'); // Decodificador para haceptar tildes y ñ
    $y = $pdf->GetY();
    $pdf->SetY($y + 5);
    ...
    $pdf->Cell(18, 7, 'No Cuota', 1, 0, 'C');
    $pdf->Cell(28, 7, 'Cuota', 1, 0, 'C');
    $pdf->Cell(28, 7, 'Capital', 1, 0, 'C');
    $pdf->Cell(28, 7, 'Interes', 1, 0, 'C');
    $pdf->Cell(28, 7, 'Otros', 1, 0, 'C');
    $pdf->Cell(28, 7, 'Saldo capital', 1, 1, 'C');
...
$pdf->SetFont('Times', '', 12);
        $pdf->SetDrawColor(0, 128, 0); // Green borders for cells
        ...
        $pdf->CellFit(28, 7, 'Q ' . number_format($infoPP[$con]["ncapita"], 2), 'RL', 0, 'C', $fill, '', 1, 0);
        $pdf->CellFit(28, 7, 'Q ' . number_format($infoPP[$con]["nintere"], 2), 'RL', 0, 'C', $fill, '', 1, 0);
        $pdf->CellFit(28, 7, 'Q ' . number_format($infoPP[$con]["OtrosPagos"], 2), 'RL', 0, 'C', $fill, '', 1, 0);
        $saldocap = $infoPP[$con]["SaldoCapital"];
        $saldocap_formatted = number_format($saldocap, 2, '.', ',');
        $pdf->CellFit(28, 7, 'Q ' . $saldocap_formatted, 'RL', 1, 'C', $fill, '', 1, 0);
    }
    ...
    $pdf->CellFit(28, 7, 'Q ' . number_format($sumkp, 2), 0, 0, 'C', 0, '', 1, 0);
    $pdf->CellFit(28, 7, 'Q ' . number_format($sumint, 2), 0, 0, 'C', 0, '', 1, 0);
    $pdf->CellFit(28, 7, 'Q ' . number_format($sumotr, 2), 0, 0, 'C', 0, '', 1, 0);
