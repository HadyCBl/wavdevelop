...
$tamanioFuente = 8;
$tamanioTitulo = 10;
$tamanio_linea = 3; //altura de la linea/celda
$ancho_linea = 49; //anchura de la linea/celda
...
$pdf->Rect(9, 47, 170, 25, 'D'); //CUADRO 1 DATOS GENERALES
$pdf->Rect(180, 47, 25, 25, 'D'); //CUADRO foto
$pdf->Rect(9, 73, 196, 26, 'D'); //CUADRO 2 DATOS GENERALES
$pdf->Rect(9, 100, 196, 31, 'D'); //CUADRO 3 DATOS GENERALES
...
$pdf->Ln(2);
$pdf->SetFont($fuente, 'B', $tamanioFuente);
$pdf->Cell($ancho_linea * 2, $tamanio_linea, 'Profesion/Oficio', 0, 1, 'C'); //Profesion titulo
$pdf->SetFont($fuente, '', $tamanioFuente);
$pdf->Cell($ancho_linea * 2, $tamanio_linea, decode_utf8($profesion ?? " "), 0, 1, 'C', true); //profesion  DATO

$pdf->Ln(5);
$pdf->SetFont($fuente, 'B', $tamanioFuente);
$pdf->Cell($ancho_linea + $espacioextra, $tamanio_linea, 'Condicion Migratoria', 0, 0, 'C'); //condicion migratoria titulo
$pdf->Cell($espacio_blanco, $tamanio_linea, ' ', 0, 0, 'C'); //espacio
$pdf->Cell($ancho_linea + $espacioextra, $tamanio_linea, 'Nacionalidad', 0, 0, 'C'); //Nacionalidad titulo
$pdf->Cell($espacio_blanco, $tamanio_linea, ' ', 0, 0, 'C'); //espacio
$pdf->Cell($ancho_linea + $espacioextra, $tamanio_linea, 'Otra nacionalidad', 0, 1, 'C'); //otra nacionalidad titulo

$pdf->Cell($ancho_linea + $espacioextra, $tamanio_linea, decode_utf8(decode_utf8($origen ?? " ")), 0, 0, 'C', true); //
$pdf->Cell($espacio_blanco, $tamanio_linea, ' ', 0, 0, 'C'); //espacio
$pdf->SetFont($fuente, '', $tamanioFuente);
$pdf->Cell($ancho_linea + $espacioextra, $tamanio_linea, decode_utf8(decode_utf8($nacionalidad ?? " ")), 0, 0, 'C', true); //Nacionalidad dato
$pdf->Cell($espacio_blanco, $tamanio_linea, ' ', 0, 0, 'C'); //espacio
$pdf->Cell($ancho_linea + $espacioextra, $tamanio_linea, decode_utf8(decode_utf8($otraNacionalidad ?? " ")), 0, 1, 'C', true); //otra nacionalidad DATO
...
switch ($tipoDocumento) {
    case '1':
        ...
        break;
    case '2':
        $tipoDocTexto = 'Pasaporte';
        break;
    case '3':
        $tipoDocTexto = 'DPI';
        break;
    default:
        $tipoDocTexto = $tipoDocumento;
        break;
}
$pdf->Cell($ancho_linea2 + $espacioextra + 6, $tamanio_linea, 'Numero de ' . $tipoDocTexto, 0, 0, 'C'); //numero de identificacion titulo
$pdf->Cell($espacio_blanco2, $tamanio_linea, ' ', 0, 0, 'C'); //espacio
$pdf->Cell($ancho_linea2 + $espacioextra, $tamanio_linea, 'Extendido', 0, 0, 'C'); //pais extendido titulo
$pdf->Cell($espacio_blanco2, $tamanio_linea, ' ', 0, 0, 'C'); //espacio
$pdf->Cell($ancho_linea2 + $espacioextra, $tamanio_linea, 'Departamento', 0, 0, 'C'); //Departamento extendido titulo
$pdf->Cell($espacio_blanco2, $tamanio_linea, ' ', 0, 0, 'C'); //espacio
$pdf->Cell($ancho_linea2 + $espacioextra, $tamanio_linea, 'Municipio', 0, 1, 'C'); //Departamento extendido titulo

$pdf->SetFont($fuente, '', $tamanioFuente);
// $pdf->Cell($ancho_linea2, $tamanio_linea, $tipoDocTexto, 0, 0, 'C', true); //doc  DATO
// $pdf->Cell($espacio_blanco2, $tamanio_linea, ' ', 0, 0, 'C'); //espacio
$pdf->Cell($ancho_linea2 + $espacioextra + 6, $tamanio_linea, $identificacion, 0, 0, 'C', true); //numero doc DATO
$pdf->Cell($espacio_blanco2, $tamanio_linea, ' ', 0, 0, 'C'); //espacio
$pdf->Cell($ancho_linea2 + $espacioextra, $tamanio_linea, $paisExtiende, 0, 0, 'C', true); //pais extendido0 DATO
$pdf->Cell($espacio_blanco2, $tamanio_linea, ' ', 0, 0, 'C'); //espacio
$pdf->Cell($ancho_linea2 + $espacioextra, $tamanio_linea, decode_utf8(karely(Departamento::obtenerNombre($deparNac))), 0, 0, 'C', true); //departamento extendido DATO
$pdf->Cell($espacio_blanco2, $tamanio_linea, ' ', 0, 0, 'C'); //espacio
$pdf->Cell($ancho_linea2 + $espacioextra, $tamanio_linea, decode_utf8(karely(Municipio::obtenerNombre($muniNac ?? 0))), 0, 1, 'C', true); //departamento extendido DATO

$pdf->Ln(2);
$pdf->SetFont($fuente, 'B', $tamanioFuente);
$pdf->Cell($ancho_linea + 2, $tamanio_linea, 'No de NIT', 0, 0, 'C'); //nit titulo
$pdf->Cell($espacio_blanco, $tamanio_linea, ' ', 0, 0, 'C'); //espacio
$pdf->Cell($ancho_linea + 25, $tamanio_linea, 'Email', 0, 0, 'C'); //email titulo
$pdf->Cell($espacio_blanco, $tamanio_linea, ' ', 0, 0, 'C'); //espacio
$pdf->Cell($ancho_linea, $tamanio_linea, 'Afiliacion IGGS', 0, 1, 'C'); //iggs  titulo
$pdf->SetFont($fuente, '', $tamanioFuente);

$pdf->Cell($ancho_linea + 2, $tamanio_linea, $noNit, 0, 0, 'C', true); //nit DATO
$pdf->Cell($espacio_blanco, $tamanio_linea, ' ', 0, 0, 'C'); //espacio
$pdf->Cell($ancho_linea + 25, $tamanio_linea, ($email ?? " "), 0, 0, 'C', true); //email  DATO
$pdf->Cell($espacio_blanco, $tamanio_linea, ' ', 0, 0, 'C'); //espacio
...
$pdf->Cell(193, $tamanio_linea, decode_utf8($direccion ?? " "), 0, 1, 'C', true); //direccion DATO
$pdf->SetFont($fuente, 'B', $tamanioFuente);
$pdf->Cell($ancho_linea + $espacioextra, $tamanio_linea, 'Telefono #1', 0, 0, 'C'); //tel1 titulo
$pdf->Cell($espacio_blanco2, $tamanio_linea, ' ', 0, 0, 'C'); //espacio
$pdf->Cell($ancho_linea + $espacioextra, $tamanio_linea, 'Telefono #2', 0, 0, 'C'); //tel2 titulo
$pdf->Cell($espacio_blanco2, $tamanio_linea, ' ', 0, 0, 'C'); //espacio
$pdf->Cell($ancho_linea + $espacioextra + 11, $tamanio_linea, 'Condicion de la vivienda', 0, 1, 'C'); //condicion vivienda titulo
$pdf->SetFont($fuente, '', $tamanioFuente);

$pdf->Cell($ancho_linea + $espacioextra, $tamanio_linea, $tel1, 0, 0, 'C', true); //tel1 DATO
$pdf->Cell($espacio_blanco2, $tamanio_linea, ' ', 0, 0, 'C'); //espacio
$pdf->Cell($ancho_linea + $espacioextra, $tamanio_linea, $tel2, 0, 0, 'C', true); //tel2 DATO
$pdf->Cell($espacio_blanco2, $tamanio_linea, ' ', 0, 0, 'C'); //espacio
$pdf->Cell($ancho_linea + $espacioextra + 11, $tamanio_linea, $condicionVivienda ?? " ", 0, 1, 'C', true); //condicion vivienda DATO
$pdf->Ln(2);

$pdf->SetFont($fuente, 'B', $tamanioFuente);
$pdf->Cell($ancho_linea + $espacioextra, $tamanio_linea, decode_utf8('Año Vivienda'), 0, 0, 'C'); //año vivienda titulo
$pdf->Cell($espacio_blanco2, $tamanio_linea, ' ', 0, 0, 'C'); //espacio
$pdf->Cell($ancho_linea + $espacioextra, $tamanio_linea, 'Zona', 0, 0, 'C'); //condicion vivienda titulo
$pdf->Cell($espacio_blanco2, $tamanio_linea, ' ', 0, 0, 'C'); //espacio
$pdf->Cell($ancho_linea + $espacioextra + 11, $tamanio_linea, 'Barrio', 0, 1, 'C'); //año vivienda titulo

$pdf->SetFont($fuente, '', $tamanioFuente);
$pdf->Cell($ancho_linea + $espacioextra, $tamanio_linea, $anioReside ?? " ", 0, 0, 'C', true); //año vivienda DATO
$pdf->Cell($espacio_blanco2, $tamanio_linea, ' ', 0, 0, 'C'); //espacio
$pdf->Cell($ancho_linea + $espacioextra, $tamanio_linea, $zona, 0, 0, 'C', true); //condicion vivienda DATO
$pdf->Cell($espacio_blanco2, $tamanio_linea, ' ', 0, 0, 'C'); //espacio
$pdf->Cell($ancho_linea + $espacioextra + 11, $tamanio_linea, $barrio, 0, 1, 'C', true); //año vivienda DATO

$pdf->Ln(2);

$pdf->SetFont($fuente, 'B', $tamanioFuente);
$pdf->Cell($ancho_linea + 71, $tamanio_linea, 'Nombre de Conyuge', 0, 0, 'C'); //nombre conyuge titulo

$pdf->Cell($espacio_blanco2, $tamanio_linea, ' ', 0, 0, 'C'); //espacio
$pdf->Cell($ancho_linea + 20, $tamanio_linea, 'Tel. de Conyuge', 0, 1, 'C'); //nombre conyuge titulo
$pdf->SetFont($fuente, '', $tamanioFuente);

$pdf->Cell($ancho_linea + 71, $tamanio_linea, Utf8::decode($conyuge ?? " "), 0, 0, 'C', true); //nombre conyuge dato
$pdf->Cell($espacio_blanco2, $tamanio_linea, ' ', 0, 0, 'C'); //espacio
$pdf->Cell($ancho_linea + 20, $tamanio_linea, $telconyuge, 0, 1, 'C', true); //nombre conyuge dato
...
// Mostrar referencias solo si hay al menos una referencia con nombre o teléfono
$hayReferencia = (
    !empty($Nombrereferencia1) || !empty($Nombrereferencia2) || !empty($Nombrereferencia3) ||
    !empty($telReferencia1) || !empty($telReferencia2) || !empty($telReferencia3)
);
...
if ($hayReferencia) {
    ...
    $pdf->SetFillColor(204, 229, 255);
    $pdf->Cell(66, $tamanio_linea, 'Referencia #1', 1, 0, 'C', true);
    $pdf->Cell(65, $tamanio_linea, 'Referencia #2', 1, 0, 'C', true);
    $pdf->Cell(65, $tamanio_linea, 'Referencia #3', 1, 1, 'C', true);
    ...
    // Names row
    $pdf->Cell(66, $tamanio_linea, decode_utf8($Nombrereferencia1), 1, 0, 'C', true);
    $pdf->Cell(65, $tamanio_linea, decode_utf8($Nombrereferencia2), 1, 0, 'C', true);
    $pdf->Cell(65, $tamanio_linea, decode_utf8($Nombrereferencia3), 1, 1, 'C', true);

    // Relationship row
    $pdf->Cell(66, $tamanio_linea, decode_utf8($parentescoRef1), 1, 0, 'C', true);
    $pdf->Cell(65, $tamanio_linea, decode_utf8($parentescoRef2), 1, 0, 'C', true);
    $pdf->Cell(65, $tamanio_linea, decode_utf8($parentescoRef3), 1, 1, 'C', true);

    // Phone row
    $pdf->Cell(66, $tamanio_linea, ($telReferencia1), 1, 0, 'C', true);
    $pdf->Cell(65, $tamanio_linea, ($telReferencia2), 1, 0, 'C', true);
    $pdf->Cell(65, $tamanio_linea, ($telReferencia3), 1, 1, 'C', true);

    // Address row
    $pdf->Cell(66, $tamanio_linea, decode_utf8($direccionRef1), 1, 0, 'C', true);
    $pdf->Cell(65, $tamanio_linea, decode_utf8($direccionRef2), 1, 0, 'C', true);
    $pdf->Cell(65, $tamanio_linea, decode_utf8($direccionRef3), 1, 1, 'C', true);

    // Address reference row
    $pdf->Cell(66, $tamanio_linea, decode_utf8($refDireccion1), 1, 0, 'C', true);
    $pdf->Cell(65, $tamanio_linea, decode_utf8($refDireccion2), 1, 0, 'C', true);
    $pdf->Cell(65, $tamanio_linea, decode_utf8($refDireccion3), 1, 1, 'C', true);
}
...
// Mostrar sección solo si hay ingresos
if (mysqli_num_rows($consultaIngresos) > 0) {
    $pdf->SetFont($fuente, 'B', $tamanioFuente);
    $pdf->SetFillColor(204, 229, 255);
    $pdf->Cell(0, 5, 'INGRESOS DEL SOLICITANTE', 0, 1, 'C', true);
    $pdf->SetFont($fuente, '', $tamanioFuente);
    $pdf->SetFont($fuente, 'B', $tamanioFuente);
    $pdf->SetFillColor(555, 255, 204);
    $pdf->Cell(35, $tamanio_linea, 'Tipo de fuente', 1, 0, 'C', true); //
    $pdf->Cell(85, $tamanio_linea, 'Nombre', 1, 0, 'C', true);
    ...
    while ($ingresos = mysqli_fetch_array($consultaIngresos, MYSQLI_ASSOC)) {
        ...
        $puesto = decode_utf8($ingresos['puesto_ocupa']);
        ...
        $pdf->Cell(85, $tamanio_linea, $nombreEmpresa, 1, 0, 'C');
...
$consultaEgresos = mysqli_query($conexion, "SELECT * FROM cli_egresos WHERE id_cliente = '{$codigo}' and estado = 1");
// Mostrar sección solo si hay egresos
if (mysqli_num_rows($consultaEgresos) > 0) {
    $pdf->SetFont($fuente, 'B', $tamanioFuente);
    $pdf->SetFillColor(204, 229, 255);
    $pdf->Cell(0, 5, 'EGRESOS DEL SOLICITANTE', 0, 1, 'C', true);
    $pdf->SetFont($fuente, '', $tamanioFuente);
    $pdf->SetFont($fuente, 'B', $tamanioFuente);
    $pdf->SetFillColor(555, 255, 204);
    $pdf->Cell(170, $tamanio_linea, '   Descripcion del Egreso', 1, 0, 'L', true); //
    $pdf->Cell(25, $tamanio_linea, 'Monto', 1, 1, 'C', true);

    $pdf->SetFont($fuente, '', $tamanioFuente);

    while ($egresos = mysqli_fetch_array($consultaEgresos, MYSQLI_ASSOC)) {
        $descripcionEgreso = decode_utf8($egresos['nombre']);
        $montoEgreso = decode_utf8($egresos['monto']);
        $pdf->Cell(170, $tamanio_linea, $descripcionEgreso, 1, 0, 'C');
        $pdf->Cell(25, $tamanio_linea, $montoEgreso, 1, 1, 'C');
    }
}
...
if ($bandera || $bandera2 || $bandera3) {
    ...
    $pdf->CellFit($ancho_linea2 * 2 + 10, $tamanio_linea, 'Descripcion', 1, 0, 'C', true, '', 1, 0);
    $pdf->CellFit($ancho_linea, $tamanio_linea, 'Cuenta', 1, 0, 'C', true, '', 1, 0);
    $pdf->CellFit($ancho_linea - 19, $tamanio_linea, 'Monto', 1, 1, 'C', true, '', 1, 0);
    $pdf->SetFont($fuente, '', $tamanioFuente);
}
...
if ($bandera) {
    foreach ($datos1 as $dato) {
        ...
        $pdf->CellFit($ancho_linea2 * 2 + 10, $tamanio_linea, decode_utf8($dato['descripcion']), 1, 0, 'C', 0, '', 1, 0);
        $pdf->CellFit($ancho_linea, $tamanio_linea, $dato['cuenta'], 1, 0, 'C', 0, '', 1, 0);
        $pdf->CellFit($ancho_linea - 19, $tamanio_linea, $dato['saldo'], 1, 1, 'C', '', '0', 1, 0);
    }
}
