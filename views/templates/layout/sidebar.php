...
try {
    ...
    // Instanciar el gestor de permisos
    $permissionManager = new ModulePermissionManager($database, $db_name_general);

    // Obtener módulos permitidos para la institución
    $modulesResult = $permissionManager->getInstitutionModules($idagencia, 'I');

    if (!$modulesResult['success']) {
        throw new SoftException($modulesResult['message']);
    }

    // Para cada módulo, obtener los permisos del usuario
    foreach ($modulesResult['data'] as $module) {
        $idmodulo = $module["id"];
        $rama = $module["rama"];

        $permissionsResult = $permissionManager->getUserModulePermissions($idusuario, $rama, $idmodulo);

        if ($permissionsResult['success']) {
            $module['permissions'] = $permissionsResult['data'];
            $showmodules[] = $module;
        }
    }
 ...
 *  route: 'api/seguros/servicios/index'  ejemplo
 * la ruta se forma asi, en modulos esta la ruta principal (campo ruta) ej seguros
 * en submenus esta el resto de la ruta (campo condi) ej servicios/index
 * 
 */
$modulesMigrados = [
    29, //id modulo auxilio postumo
...
<aside
    ...
    class="sidebar fixed left-0 top-0 z-[9999] flex h-screen w-[280px] flex-col overflow-hidden bg-white dark:bg-[#0A0E27] shadow-2xl lg:static lg:translate-x-0 border-r border-gray-200 dark:border-white/5">

    <!-- SIDEBAR HEADER -->
    <div class="flex flex-col items-center py-8 px-4 border-b border-gray-200 dark:border-white/10">
        <a href="<?= $makeUrl ?>" class="flex flex-col items-center gap-4 group">
            ...
            <span
                ...
                class="text-sm font-bold text-gray-800 dark:text-white text-center leading-tight tracking-wide">
    ...
    <div class="flex flex-col flex-1 overflow-y-auto sidebar-scroll py-6">
        ...
        <nav x-data="{selected: $persist('Dashboard')}" class="space-y-6 px-4">
            ...
            <div>
                <h3 class="text-[10px] font-bold uppercase tracking-wider text-gray-400 dark:text-white/40 mb-3 px-3"
                ...
                <ul class="space-y-1">
                    ...
                    <li>
                        <a
                            ...
                            :class="(selected === 'Dashboard') || (page === 'Dashboard') ? 'bg-gradient-to-r from-blue-600 to-blue-500 text-white shadow-lg shadow-blue-500/30' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/5 hover:text-gray-900 dark:hover:text-white'">
                            <div class="flex items-center justify-center w-5 h-5">
                                <svg
                                    class="w-5 h-5 transition-transform duration-200 group-hover:scale-110"
                                    :class="(selected === 'Dashboard') || (page === 'Dashboard') ? 'fill-white' : 'fill-gray-500 dark:fill-gray-400 group-hover:fill-blue-500'"
                                    viewBox="0 0 24 24"
                                ...
                                <li>
                                    <details :open="selected === '<?= $identificadorModulo ?>'" @toggle="selected = $event.target.open ? '<?= $identificadorModulo ?>' : ''" class="group">
                                        <summary class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 cursor-pointer list-none"
                                            :class="selected === '<?= $identificadorModulo ?>' ? 'bg-blue-500/10 text-blue-500 dark:text-blue-400' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/5 hover:text-gray-900 dark:hover:text-white'">
                                            <div class="flex items-center justify-center w-5 h-5">
                                                <svg class="w-5 h-5 transition-all duration-200 group-hover:scale-110" :class="selected === '<?= $identificadorModulo ?>' ? 'fill-blue-500 dark:fill-blue-400' : 'fill-gray-500 dark:fill-gray-400 group-hover:fill-blue-500'" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                    <?= $iconModulo ?>
                                                </svg>
                                            </div>
                                            <span :class="sidebarToggle ? 'lg:hidden' : ''" class="flex-1"><?= $descripcionModulo ?></span>
                                            <svg class="w-4 h-4 transition-transform duration-200 shrink-0" :class="selected === '<?= $identificadorModulo ?>' ? 'rotate-180 text-blue-500 dark:text-blue-400' : 'text-gray-500 dark:text-gray-400'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                ...
                                                <li>
                                                    <details :open="subSelected === '<?= $identificadorMenu ?>' || JSON.parse('<?= $opcionesSubmenuNivel2Json ?>').includes(activeItem)" @toggle="subSelected = $event.target.open ? '<?= $identificadorMenu ?>' : ''" class="group">
                                                        <summary class="flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-medium transition-all duration-200 cursor-pointer list-none"
                                                            :class="(subSelected === '<?= $identificadorMenu ?>' || JSON.parse('<?= $opcionesSubmenuNivel2Json ?>').includes(activeItem)) ? 'text-blue-500 dark:text-blue-400 bg-blue-500/10' : 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-300'">
                                                        ...
                                                        <ul class="mt-1 ml-4 space-y-0.5">
                                                            <?php foreach ($menuPrincipal['submenus'] as $submenu) :
                                                                if (in_array($idModuloPrincipal, $modulesMigrados)) {
                                                                    $route = "api/" . $ruta . "/" . $submenu['condi'];
                                                                    $onclickAction = sprintf(
                                                                        "loadModuleView('#cuadro', { route: '%s' });",
                                                                        $route,
                                                                    );
                                                                    // Log::debug("Módulo migrado detectado. Ruta generada: $route");
                                                                } else {
                                                            ...
                                                            ?>
                                                                <li>
                                                                    <a href="#"
                                                                        ...
                                                                        :class="activeItem === '<?= $opcionId ?>' ? 'text-blue-500 dark:text-blue-400 bg-blue-500/10 font-medium' : 'text-gray-600 dark:text-gray-500 hover:text-gray-900 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/5'">
                                                                        <span class="flex items-center gap-2">
                                                                            <span class="w-1 h-1 rounded-full transition-colors" :class="activeItem === '<?= $opcionId ?>' ? 'bg-blue-500 dark:bg-blue-400' : 'bg-gray-400 dark:bg-gray-600'"></span>
                                                                            <?= $submenu['caption'] ?>
                                                                        </span>
                                                                        <span x-show="activeItem === '<?= $opcionId ?>'" class="w-1.5 h-1.5 rounded-full bg-blue-500 dark:bg-blue-400 animate-pulse"></span>
                                                                    </a>
                                                                </li>
                                                            <?php endforeach; ?>
                                                        </ul>
            ...
            <div class="mt-auto pt-6 border-t border-gray-200 dark:border-white/10">
                ...
                <ul class="space-y-1">
                    <li>
                        <!-- <button onclick="loadModuleView('#cuadro', {
                        ...
                        <a
                            href="<?= BASE_URL . "/views/" ?>"
                            class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/5 hover:text-gray-900 dark:hover:text-white group border border-gray-200 dark:border-white/10 hover:border-blue-500/50">
