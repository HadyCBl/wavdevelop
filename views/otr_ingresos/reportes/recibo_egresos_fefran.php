...
function recibo($pdf, $registro, $hoy, $usuario, $info)
{
    ...
    if ($registro[0]["tipomov"] == "INGRESO") {
        ...
        $pdf->CellFit($wlinea * 3, $hlinea, $nombrecliente, 0, 0, 'R', 0, '', 1, 0);
        $pdf->CellFit(0, $hlinea, "RECIBO DE " . $registro[0]['tipomov'] . ": " . $registro[0]['recibo'], 0, 0, 'R', 0, '', 1, 0);

        $pdf->ln($hlinea);

        $pdf->CellFit($wlinea * 2, $hlinea, "FECHA: " . setdatefrench($registro[0]['fecha']), 0, 0, 'L', 0, '', 1, 0);
        $pdf->CellFit(0, $hlinea, "DIRECCION: " . utf8_decode($info[0]['muni_lug']), 0, 0, 'R', 0, '', 1, 0);
