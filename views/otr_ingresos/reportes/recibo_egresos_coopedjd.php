...
$consulta = "SELECT tp.recibo AS recibo, tp.cliente AS cli, tp.fecha AS fecha, IF(tpi.tipo = 1, 'INGRESO', 'EGRESO') AS tipomov, tpi.nombre_gasto AS detalle, tpm.monto AS monto, tp.agencia AS agencia, descripcion,tp.tipoadicional FROM otr_pago_mov tpm 
...
function recibo($pdf, $registro, $hoy, $usuario, $info)
{
    ...
    $x = 165;
    $pdf->SetX($x);
    $pdf->CellFit(0, $hlinea, $hoy, 0, 0, 'L', 0, '', 1, 0);

    $pdf->ln($hlinea);

    $pdf->SetFont($fuente, '', $fontSize);
    $nombrecliente = ($registro[0]['tipoadicional'] == 2) ? "CLIENTE: " . decode_utf8($registro[0]['cli']) : " ";
    $pdf->CellFit($wlinea * 3, $hlinea, $nombrecliente, 0, 0, 'L', 0, '', 1, 0);
    $pdf->CellFit(0, $hlinea, "RECIBO DE " . $registro[0]['tipomov'] . ": " . $registro[0]['recibo'], 0, 0, 'L', 0, '', 1, 0);
    ...
    $pdf->CellFit(0, $hlinea, "DIRECCION: " . decode_utf8(karely(Municipio::obtenerNombrePorCodigo($info[0]['municipio']))), 0, 0, 'L', 0, '', 1, 0);
    ...
    $pdf->SetFont($fuente, 'B', $fontSize);
    $pdf->CellFit($wlinea * 2, $hlinea, 'TOTAL', 0, 0, 'C', 0, '', 1, 0);
    $pdf->CellFit($wlinea * 2, $hlinea, number_format($total, 2), 0, 0, 'R', 0, '', 1, 0);
    ...
    $pdf->MultiCell(0, $hlinea,'CONCEPTO: '. decode_utf8($registro[0]['descripcion']), 0, 'L');

    $pdf->ln(0);

    $pdf->CellFit(0, $hlinea, 'ID:' . $usuario . ' USUARIO:' . decode_utf8($_SESSION['nombre']) . ' ' . decode_utf8($_SESSION['apellido']), 0, 0, 'C', 0, '', 1, 0);
}
