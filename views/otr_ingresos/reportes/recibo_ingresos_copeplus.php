...
switch ($tipo) {
    case 'xlsx';
        ...
        break;
    case 'pdf':
        printpdf($data, $info, $idusuario, $usuarioag);
        // printpdf2($data, $info, $idusuario);
        break;
}
function printpdf($registro, $info, $id_usu, $usuarioag)
{
    ...
    if ($usuarioag == 1) {
        ...
        recibo2($pdf, $registro, $hoy, $id_usu, $info);
    } else {
        $pdf->Cell(0, 10, "Agencia no reconocida: $usuarioag", 0, 1, 'C');
    }
...
function recibo($pdf, $registro, $hoy, $usuario, $info)
{
    ...
    $pdf->CellFit($wlinea * 3, $hlinea, "CLIENTE: " . decode_utf8($registro[0]['cli']), 0, 0, 'R', 0, '', 1, 0);
    ...
    $pdf->CellFit(0, $hlinea, "DIRECCION: " . decode_utf8($info[0]['muni_lug']), 0, 0, 'R', 0, '', 1, 0);
...
function recibo2($pdf, $registro, $hoy, $usuario, $info)
{
    /**
     * CellFit($w, $h = 0, $txt = '', $border = 0, $ln = 0, $align = '', $fill = false, $link = '', $scale = false, $force = true)
     *
