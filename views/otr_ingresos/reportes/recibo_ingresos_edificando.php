...
function printpdf($registro, $info, $id_usu, $conexion)
{
    ...
    $pdf->SetX(0);
    $pdf->CellFit($ancho_linea2 + 110, $tamanio_linea + 1, 'RECIBO DE ' . $registro[0]['tipomov'] . ': ', 0, 0, 'R', 0, '', 1, 0);
    ...
    //NOMBRE DEL CLIENTE
    $texto = "<p>NOMBRE DE CLIENTE: <vb><pers><place>" . decode_utf8($registro[0]['cli'])  . "</place></pers></vb></p>";
    // $pdf->SetX(0);
    $pdf->WriteTag(0, 5, $texto, 0, "C", 0, 0);
    $pdf->Ln(2);

    //FECHA
    $texto = "<p>FECHA: <vb><pers><place>" . date("d-m-Y", strtotime($registro[0]['fecha'])) . "</place></pers></vb>        DIRECCION: <vb><pers><place>" . decode_utf8(karely(Municipio::obtenerNombrePorCodigo($info[0]['municipio']))). "</place></pers></vb></p>";
    $pdf->WriteTag(0, 5, decode_utf8($texto), 0, "C", 0, 0);
    $pdf->Ln(2);

    $pdf->SetFont($fuente, 'B', 11);
    $pdf->SetX(35);
    $pdf->CellFit($ancho_linea2 + 45, $tamanio_linea + 1, decode_utf8('DESCRIPCIÓN'), 0, 0, 'C', 0, '', 1, 0);
    $pdf->Ln(3);
    ...
    foreach ($registro as $key => $value) {
        ...
        $pdf->CellFit($ancho_linea2 - 40, $tamanio_linea + 1, ' ', 0, 0, 'L', 0, '', 1, 0);
    ...
    $pdf->CellFit($ancho_linea2 * 7, $tamanio_linea + 1, decode_utf8('TOTAL'), 0, 0, 'C', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 - 20, $tamanio_linea + 1, ' ', 0, 0, 'L', 0, '', 1, 0);
