...
function printpdf($registro, $info, $id_usu, $conexion)
{
    ...
    //NOMBRE DEL CLIENTE
    $texto = "<p>NOMBRE DE CLIENTE: <vb><pers><place>" .Utf8::decode($registro[0]['cli'])  . "</place></pers></vb></p>";
    ...
    $pdf->WriteTag(0, 5, Utf8::decode($texto), 0, "C", 0, 0);
    $pdf->Ln(4);

    $pdf->SetFont($fuente, 'B', 11);
    $pdf->CellFit($ancho_linea2 + 45, $tamanio_linea + 1, Utf8::decode('DESCRIPCIÓN'), 0, 0, 'C', 0, '', 1, 0);
    $pdf->Ln(5);
    ...
    foreach ($registro as $key => $value) {
        ...
        $pdf->CellFit($ancho_linea2 + 60, $tamanio_linea + 1, Utf8::decode($value['detalle']), 0, 0, 'L', 0, '', 1, 0);
    ...
    $pdf->CellFit($ancho_linea2 + 60, $tamanio_linea + 1, Utf8::decode('TOTAL'), 0, 0, 'C', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 - 20, $tamanio_linea + 1, ' ', 0, 0, 'L', 0, '', 1, 0);
    ...
    $pdf->MultiCell(0, $tamanio_linea + 1, Utf8::decode($format_monto->toMoney($decimal[0], 2, '', '')) . $res . "/100", 0, 'C');
    $pdf->Ln(5);
    ...
    $pdf->CellFit(0, $tamanio_linea + 1, 'ID:' . $id_usu . ' USUARIO:' . Utf8::decode(Utf8::decode($_SESSION['nombre'])) . ' ' . Utf8::decode(Utf8::decode($_SESSION['apellido'])), 0, 0, 'C', 0, '', 1, 0);
