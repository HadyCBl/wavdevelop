...
function printpdf($registro, $info, $id_usu)
{
    ...
    recibo($pdf, $registro, $hoy, $id_usu, $info);
...
function recibo($pdf, $registro, $hoy, $usuario, $info)
{
    ...
    // $pdf->CellFit(0, $hlinea, $hoy, 0, 0, 'L', 0, '', 1, 0); // Cambiado 'R' a 'L'

    $pdf->ln($hlinea * 2);

    $pdf->SetFont($fuente, '', $fontSize);
    $pdf->CellFit($wlinea * 2, $hlinea, "RECIBO: " . $registro[0]['recibo'], 0, 1, 'L', 0, '', 1, 0);
    $pdf->CellFit($wlinea * 2, $hlinea, "FECHA: " . setdatefrench($registro[0]['fecha']), 0, 1, 'L', 0, '', 1, 0);
    $pdf->CellFit($wlinea * 3, $hlinea, "NOMBRE: " . decode_utf8($registro[0]['cli']), 0, 1, 'L', 0, '', 1, 0);
    $pdf->CellFit($wlinea * 3, $hlinea, "CODIGO-CLI: ", 0, 1, 'L', 0, '', 1, 0);
    $pdf->CellFit($wlinea * 3, $hlinea, "No.CUENTA: " , 0, 1, 'L', 0, '', 1, 0);
    $pdf->CellFit($wlinea * 3, $hlinea, "TIPO TRANSACCION: " .  $registro[0]['tipomov'].", ".decode_utf8($registro[0]['descripcion']), 0, 1, 'L', 0, '', 1, 0);
    $pdf->ln($hlinea);
    foreach ($registro as $value) {
        $pdf->CellFit($wlinea * 2, $hlinea, decode_utf8($value['detalle']), 0, 1, 'L', 0, '', 1, 0);
        $pdf->CellFit($wlinea * 2, $hlinea, "MONTO: ". number_format(round($value['monto'], 2), 2), 0, 1, 'L', 0, '', 1, 0);
        $pdf->ln($hlinea);
    }
    $total = round((array_sum(array_column($registro, 'monto'))), 2);

    $pdf->SetFont($fuente, 'B', $fontSize);
    $pdf->CellFit($wlinea * 2, $hlinea, 'TOTAL:' . number_format($total, 2), 0, 1, 'C', 0, '', 1, 0);
    
    $pdf->ln($hlinea * 3);
    $pdf->SetFont($fuente, '', $fontSize);
    
    // Firmas
    $pdf->CellFit($wlinea * 2, $hlinea, '_' . str_repeat('_', 20), 0, 0, 'C', 0, '', 1, 0);
    $pdf->CellFit($wlinea * 2, $hlinea, '_' . str_repeat('_', 20), 0, 1, 'C', 0, '', 1, 0);
    
    $pdf->CellFit($wlinea * 2, $hlinea, 'F.'. decode_utf8($registro[0]['cli']), 0, 0, 'C', 0, '', 1, 0);
    $pdf->CellFit($wlinea * 2, $hlinea, 'F.'. Utf8::decode($_SESSION['nombre']), 0, 1, 'C', 0, '', 1, 0);
    ...
    // $pdf->MultiCell(0, $hlinea, decode_utf8($format_monto->toMoney($decimal[0], 2, '', '')) . $res . "/100", 0, 'C');
    $pdf->ln($hlinea);
    // $pdf->MultiCell(0, $hlinea, decode_utf8($registro[0]['descripcion']), 0, 'L');
    $pdf->ln($hlinea);
    // $pdf->CellFit(0, $hlinea, 'ID:' . $usuario, 0, 0, 'C', 0, '', 1, 0);
}