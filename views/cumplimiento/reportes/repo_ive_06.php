...
// ============================================================================
// CONSTRUCCIÓN DE CONSULTA SQL - CONTRATISTAS/PROVEEDORES DEL ESTADO (CPE)
// ============================================================================
$filtro_agencia = ($radios[0] == "anyofi") ? " AND cli.agencia = " . intval($selects[0]) : "";
$filtro_cpe = " AND cli.CPE = 'Si'";

if ($tipofecha === 'corte') {
    $strquery = "
    -- CPE CON PRODUCTOS DE AHORRO
    SELECT 
    ...
    -- CPE CON PRODUCTOS DE APORTACIÓN
    SELECT 
        cli.compl_name AS nombre,
        cli.no_identifica AS dpi,
    ...
    SELECT 
        ...
        IFNULL(tip.nombre, 'Cuenta Ahorro') AS nombre_tipo_cuenta,
        aho.fecha_apertura AS fecha_apertura,
        IFNULL(aho.monto_apertura, 0) AS monto_apertura,
        calcular_saldo_aho_tipcuenta(aho.ccodaho, ?) AS saldo,
        CONCAT(ag.cod_agenc, ' - ', ag.nom_agencia) AS agencia
    FROM ahomcta aho
    INNER JOIN tb_cliente cli ON aho.ccodcli = cli.idcod_cliente
...
try {
    ...
    if (empty($result)) {
        $showmensaje = true;
        throw new Exception("No se encontraron personas CPE con productos activos para el periodo seleccionado");
    }
...
function printxls($registro, $titlereport, $usuario, $info)
{
    ...
    $sheet->setCellValue('A2', 'REPORTE IVE - CONTRATISTAS/PROVEEDORES DEL ESTADO (CPE)');
    ...
    $sheet->setCellValue('D' . $fila, 'NÚMERO');
    $sheet->setCellValue('E' . $fila, 'NOMBRE CUENTA/CRÉDITO');
    ...
    foreach ($registro as $reg) {
        // Para Excel usar datos UTF-8 directamente, sin decode_utf8
        ...
        // Usar floatval para que sean números, no texto
    ...
    // Formato de número para columnas de montos
    ...
    echo json_encode([
        ...
        'namefile' => "Repo_CPE_IVE" . str_replace(' ', '_', $titlereport),
        'tipo' => "vnd.ms-excel",
...
function printpdf($registro, $titlereport, $info)
{
    ...
    $pdf->Cell(0, 6, decode_utf8('REPORTE IVE - CONTRATISTAS/PROVEEDORES DEL ESTADO (CPE)'), 0, 1, 'C');
    ...
    $pdf->Cell(18, 6, decode_utf8('NUMERO'), 1, 0, 'C', true);
