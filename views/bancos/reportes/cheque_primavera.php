...
function cheque_bancoindustrial($pdf, $registro, $info)
{
    ...
    $pdf->CellFit($ancho_linea + 107, $tamanio_linea + 1, "**** " . Utf8::decode(mb_strtoupper($registro[0]['nomchq'], 'utf-8')) . " ****", 0, 0, 'L', 0, '', 1, 0);
    $pdf->Ln(9);
    //numero en letras
    ...
    $pdf->MultiCell($ancho_linea + 105, $tamanio_linea + 1, "**** " . Utf8::decode($format_monto->toMoney($decimal[0], 2, '', '')) . $res . "/100 ****", 0, 'L');
    $pdf->Ln(11);
    //no negociable
    ...
    //glosa
    $pdf->SetFont($fuente, 'B', 13);
    $pdf->MultiCell($ancho_linea + 120, $tamanio_linea + 1, Utf8::decode(mb_strtoupper($registro[0]['glosa'], 'utf-8')), 0, 'L');
    $pdf->Ln(8);
    //nombre
    $pdf->SetFont($fuente, 'B', 13);
    $pdf->CellFit($ancho_linea, $tamanio_linea + 1, " ", 0, 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea + 130, $tamanio_linea + 1, "**** " . Utf8::decode(mb_strtoupper($registro[0]['nomchq'], 'utf-8')) . " ****", 0, 0, 'L', 0, '', 1, 0);
    $pdf->Ln(5);
    $pdf->SetFont($fuente, 'B', 13);
    ...
    for ($i = 0; $i < count($registro); $i++) {
        ...
        $pdf->CellFit($ancho_linea + 50, $tamanio_linea + 1, Utf8::decode(mb_strtoupper($registro[$i]['cdescrip'], 'utf-8')), 0, 0, 'L', 0, '', 1, 0);
        $pdf->SetFont($fuente, 'B', 13);
...
function cheque_banrural($pdf, $registro, $info)
{
    ...
    $pdf->SetY(10);
    $pdf->CellFit(0, $tamanio_linea + 1, $info[0]['nomb_comple'], 0, 0, 'C', 0, '', 1, 0);
    //fecha y monto
    $pdf->Ln(19);
    $pdf->SetFont($fuente, 'B', 13);
    $fechaSegundos = strtotime($registro[0]['fecdoc']);
    ...
    $pdf->SetFont($fuente, 'B', 12);
    $pdf->CellFit($ancho_linea - 20, $tamanio_linea + 1, ' ', 0, 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea + 22, $tamanio_linea + 1, 'Maximiliano Calmo', 0, 0, 'C', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea - 23, $tamanio_linea + 1, ' ', 0, 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea + 22, $tamanio_linea + 1, 'Magdalena Juan Lorenzo', 0, 0, 'C', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea - 23, $tamanio_linea + 1, ' ', 0, 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea + 22, $tamanio_linea + 1, 'Efrain Baltazar Francisco', 0, 0, 'C', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea - 20, $tamanio_linea + 1, ' ', 0, 0, 'L', 0, '', 1, 0);
    $pdf->Ln(22);
