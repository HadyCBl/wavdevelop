...
function cheque_bancoindustrial($pdf, $registro, $info)
{
    ...
    //nombre de cheque
    $pdf->CellFit($mleft, $tamanio_linea, " ", 0, 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea + 107, $tamanio_linea, "**** " . decode_utf8(mb_strtoupper($registro[0]['nomchq'], 'utf-8')) . " ****", 0, 0, 'L', 0, '', 1, 0);
    $pdf->Ln(7);
    //numero en letras
    ...
    $pdf->MultiCell($ancho_linea + 105, $tamanio_linea, "**** " . decode_utf8($format_monto->toMoney($decimal[0], 2, '', '')) . $res . "/100 ****", 0, 'L');
    $pdf->Ln(10);
    //no negociable
...
function cheque_banrural($pdf, $registro, $info)
{
    ...
    $pdf->SetY(10);
    $pdf->CellFit(0, $tamanio_linea + 1, decode_utf8(decode_utf8($info[0]['nomb_comple'])), 0, 0, 'C', 0, '', 1, 0);
    //fecha y monto
    $pdf->Ln(19);
    $pdf->SetFont($fuente, 'B', 13);
    $fechaSegundos = strtotime($registro[0]['fecdoc']);
