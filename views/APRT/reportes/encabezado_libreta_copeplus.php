...
while ($dat = mysqli_fetch_array($data, MYSQLI_ASSOC)) {
    ...
    $direccion = mb_convert_encoding($dat["Direccion"], 'UTF-8', 'ISO-8859-1');
    if ($direccion == "") {
...
$pdf->Text($x+10, $y - 1, mb_convert_encoding($nombre, 'ISO-8859-1', 'UTF-8'));
$fecha_apertura = date("d-m-Y", strtotime($fecha_apertura));
$pdf->Text($x + $espaciosegundacol, $y, $fecha_apertura);

$pdf->Text($x + 10 , $y + $espacio, $cuenta);

$pdf->Text($x + $espaciosegundacol, $y +7, $dpi);
$pdf->Text($x  +10, $y + 4, mb_convert_encoding($direccion, 'ISO-8859-1', 'UTF-8'));
