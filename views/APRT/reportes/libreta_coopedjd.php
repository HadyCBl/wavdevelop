...
switch ($condi) {
    case 'lib':
        ...
        $saldoact = 0;
        $dataact =  mysqli_query($conexion, "SELECT `monto`,`ctipope` FROM `aprmov` WHERE cestado!=2 AND `ccodaport`='$ccodaport' AND `nlibreta`=$libreta AND `lineaprint`='S'");
        ...
        $confirma = "0";
        $array[] = [];
        $noprint =  mysqli_query($conexion, "SELECT `dfecope`,`cnumdoc`,`ctipope`,`monto`,`numlinea`,`correlativo`,crazon,saldo_aportacion(aprmov.ccodaport, aprmov.dfecope ,aprmov.correlativo) AS saldo 
        FROM `aprmov` WHERE cestado!=2 AND `ccodaport`='$ccodaport' AND `nlibreta`=$libreta AND `lineaprint`='N'");
