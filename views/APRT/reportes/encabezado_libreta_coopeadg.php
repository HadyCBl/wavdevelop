...
$data = mysqli_query($conexion, "SELECT * FROM `tb_cliente` WHERE `idcod_cliente` = '$idcli'");
...
$pdf->Text($x, $y-2, mb_convert_encoding($nombre, 'ISO-8859-1', 'UTF-8'));
$pdf->Text($x+5, $y-13, mb_convert_encoding($cuenta, 'ISO-8859-1', 'UTF-8'));
$fecha_apertura = date("d-m-Y", strtotime($fecha_apertura));
$pdf->Text($x + $espaciosegundacol-10, $y-2, $idcli);
$pdf->Text($x-15, $y + $espacio, $fecha_apertura);
