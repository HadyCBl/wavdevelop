...
switch ($condi) {
    case 'lib':
        ...
        $monto = 0;
        $saldoant = 0;
        $data = mysqli_query($conexion, "SELECT `monto`,`ctipope` FROM `ahommov` WHERE `ccodaho`='$ccodaho' AND cestado!=2 AND `nlibreta`!=$libreta");
        ...
        $saldoact = 0;
        $dataact = mysqli_query($conexion, "SELECT `monto`,`ctipope` FROM `ahommov` WHERE `ccodaho`='$ccodaho' AND cestado!=2 AND `nlibreta`=$libreta AND `lineaprint`='S'");
        ...
        $confirma = "0";
        $array[] = [];
        $noprint = mysqli_query($conexion, "SELECT `dfecope`,`cnumdoc`,`ctipope`,`monto`,`numlinea`,`correlativo`,saldo_ahorro(ccodaho, dfecope ,correlativo) AS saldo FROM `ahommov` WHERE `ccodaho`='$ccodaho'  AND cestado!=2 AND `nlibreta`=$libreta AND `lineaprint`='N'");
