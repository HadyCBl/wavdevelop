...
$data = mysqli_query($conexion, "SELECT `short_name`, `no_identifica`,`tel_no1`,`Direccion` FROM `tb_cliente` WHERE `idcod_cliente` = '$idcli'");
...
$pdf->Text($x - 2, $y+1, mb_convert_encoding($nombre, 'ISO-8859-1', 'UTF-8'));
$pdf->Text($x + $espaciosegundacol, $y+1, $fecha_apertura);
$pdf->Text($x + 2, $y + $espacio+3, mb_convert_encoding($direccion, 'ISO-8859-1', 'UTF-8'));
$pdf->Text($x + 15, $y + $espacio+9, $cuenta);
$pdf->Text($x + $espaciosegundacol, $y + $espacio+9, $dpi);
