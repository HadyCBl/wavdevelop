...
switch ($condi) {
    case 'lib':
        ...
        $confirma = "0";
        $array[] = [];
        $noprint =  mysqli_query($conexion, "SELECT `dfecope`,`cnumdoc`,`ctipope`,`monto`,`numlinea`,`correlativo`,saldo_ahorro(mov.ccodaho, dfecope ,correlativo) AS saldo, usu.id_agencia, ifnull(usu2.id_agencia,1) as agencia_libreta
FROM `ahommov` mov 
...
WHERE mov.cestado!=2 AND mov.ccodaho='$ccodaho' AND mov.nlibreta=$libreta AND `lineaprint`='N'");
