...
$pdf->Text($x - 5, $y + 1, mb_convert_encoding($nombre, 'ISO-8859-1', 'UTF-8'));
$pdf->Text($x + 70, $y + 1, $dpi);
$fecha_apertura = date("d-m-Y", strtotime($fecha_apertura));
$fecha = DateTime::createFromFormat('d-m-Y', $fecha_apertura);
$dia = $fecha->format('d');
$mes = $fecha->format('m');
$anio = $fecha->format('Y');

$pdf->Text($x + 74, $y + 7, "  ".$dia. "     " . $mes . "    " . $anio);
$pdf->Text($x, $y + 7, $cuenta);
