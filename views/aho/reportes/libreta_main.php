...
switch ($condi) {
    case 'lib':
        ...
        $monto = 0;
        $saldoant = 0;
        $data = mysqli_query($conexion, "SELECT `monto`,`ctipope`,`ctipdoc` FROM `ahommov` WHERE cestado!=2 AND `ccodaho`='$ccodaho' AND `nlibreta`!='$libreta'");
        ...
        $confirma = "0";
        $array[] = [];
        $noprint = mysqli_query($conexion, "SELECT `ctipdoc`,`dfecope`,`cnumdoc`,`ctipope`,`monto`,`numlinea`,`correlativo`,crazon,saldo_ahorro(ccodaho, dfecope ,correlativo) AS saldo, usu.id_agencia FROM `ahommov` mov INNER JOIN tb_usuario usu ON usu.id_usu = mov.codusu WHERE cestado!=2 AND `ccodaho`='$ccodaho' AND `nlibreta`='$libreta' AND `lineaprint`='N'");
        ...
        //en caso sea 4 omitir xd mas que todo es para pruebas
        if ($idusuario != 4) {
            if ($confirma == "1") {
