...
switch ($condi) {
    case 'lib':
        ...
        $confirma = "0";
        $array[] = [];
        $noprint = mysqli_query($conexion, "SELECT `dfecope`,`cnumdoc`,`ctipope`,`monto`,`numlinea`,`correlativo`,`crazon`,saldo_ahorro(ccodaho, dfecope ,correlativo) AS saldo FROM `ahommov` WHERE cestado!=2 AND `ccodaho`='$ccodaho' AND `nlibreta`='$libreta' AND `lineaprint`='N'");
