...
switch ($condi) {
    case 'lib':
        ...
        $confirma = "0";
        $array[] = [];
        $noprint = mysqli_query($conexion, "
                                    SELECT ahommov.dfecope, ahommov.cnumdoc, ahommov.ctipope, ahommov.monto, ahommov.numlinea, 
                                        ahommov.correlativo, tb_usuario.id_usu, tb_usuario.id_agencia,saldo_ahorro(ccodaho, dfecope ,correlativo) AS saldo
                                    FROM ahommov
                                    INNER JOIN tb_usuario ON tb_usuario.id_usu = ahommov.codusu
                                    WHERE cestado != 2 
                                    AND ccodaho = " . $ccodaho . " 
                                    AND nlibreta = " . $libreta . " 
                                    AND lineaprint = 'N'
                                ");
