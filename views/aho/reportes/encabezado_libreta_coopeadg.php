...
$data = mysqli_query($conexion, "SELECT `short_name`, `no_identifica`, `control_interno` FROM `tb_cliente` WHERE `idcod_cliente` = '$idcli'");
...
$controlinter = ($dat["control_interno"]);
...
$pdf->Text($x - 10,$y + $espacio - 5, 'Nombre de Asociado: ' . mb_convert_encoding($nombre, 'ISO-8859-1', 'UTF-8'));
$pdf->Text($x + 15,$y + $espacio-15, $cuenta);
$fecha_apertura = date("d-m-Y", strtotime($fecha_apertura));
$pdf->Text($x - 10,$y + $espacio - 1,'No. de Asociado: '. $controlinter);
$pdf->Text($x - 10, $y + $espacio + 3, 'Fecha de Apertura: ' . $fecha_apertura);
// $pdf->Text($x-8 + $espaciosegundacol-15, $y + $espacio, $dpi);
