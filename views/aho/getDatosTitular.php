...
try {
    ...
    $database->openConnection(); // Retorna un objeto PDO
    ...
    $data = $database->getAllResults($sql, [$cuenta]);

    if ($data) {
        // Armar la respuesta con los campos necesarios
        $data = [
            'status'      => 'ok',
            'dpi'         => $data[0]['no_identifica']   ?? '',
            'nombre1'     => $data[0]['primer_name']     ?? '',
            'nombre2'     => $data[0]['segundo_name']    ?? '',
            'nombre3'     => $data[0]['tercer_name']     ?? '',
            'apellido1'   => $data[0]['primer_last']     ?? '',
            'apellido2'   => $data[0]['segundo_last']    ?? '',
            'apellido3'   => $data[0]['casada_last']     ?? '',
