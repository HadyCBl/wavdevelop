...
function printpdf($registro, $datos, $info, $cuentasingreso, $cuentasegreso, $nivelinit, $nivelfin)
{
    ...
    $pdf->MultiCell(0, 4, strtoupper(utf8_decode('El infrascrito perito contador MAXIMILIANO CALMO LÓPEZ, con número de registro 73476773 ante la Superintendencia de Administración Tributaria certifica: que el estado de resultados que antecede muestra resultado de las operaciones de la cooperativa por el periodo comprendido ' . $datos[0] . ', el cual fue obtenido de los registros contables de la entidad.')));

    // $pdf->firmas(2, ['CONTADOR', 'REPRESENTANTE LEGAL']);
    //LUGAR DE FIRMAS
    $pdf->Ln(20);
    $pdf->CellFit($ancho_linea2, $tamanio_linea, ' ', 0, 0, 'C', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 * 2, $tamanio_linea, utf8_decode('MAXIMILIANO CALMO LÓPEZ'), 'T', 0, 'C', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2, $tamanio_linea, ' ', 0, 0, 'C', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 * 2, $tamanio_linea, utf8_decode('EFRAIN BATAZAR FRANCISCO'), 'T', 1, 'C', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2, $tamanio_linea, ' ', 0, 0, 'C', 0, '', 1, 0);
