...
function printpdf($registro, $datos, $info, $nivelinit, $nivelfin,  $cuentasactivo, $cuentaspasivo, $cuentascapital, $reguladorasactivo, $cuentacontableer, $resultadoer)
{
    ...
    $pdf->Ln(4);
    $pdf->MultiCell(0, 4, strtoupper(Utf8::decode('El infrascrito perito contador MAXIMILIANO CALMO LÓPEZ, con número de registro 73476773 ante la Superintendencia de Administración Tributaria certifica: que el balance general que antecede muestra la situación financiera de la cooperativa por el periodo comprendido ' . $datos[0] . ', el cual fue obtenido de los registros contables de la entidad.')));

    // $pdf->firmas(2, ['CONTADOR', 'REPRESENTANTE LEGAL']);
    //LUGAR DE FIRMAS
    $pdf->Ln(20);
    $pdf->CellFit($ancho_linea2, $tamanio_linea, ' ', 0, 0, 'C', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 * 2, $tamanio_linea, Utf8::decode('MAXIMILIANO CALMO LÓPEZ'), 'T', 0, 'C', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2, $tamanio_linea, ' ', 0, 0, 'C', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 * 2, $tamanio_linea, Utf8::decode('EFRAIN BALTAZAR FRANCISCO'), 'T', 1, 'C', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2, $tamanio_linea, ' ', 0, 0, 'C', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 * 2, $tamanio_linea, 'CONTADOR GENERAL', '', 0, 'C', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2, $tamanio_linea, ' ', 0, 0, 'C', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 * 2, $tamanio_linea, 'REPRESENTANTE LEGAL', '', 1, 'C', 0, '', 1, 0);
    $pdf->Ln(10);
