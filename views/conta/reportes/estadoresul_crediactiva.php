...
function printpdf($registro, $datos, $info, $cuentasingreso, $cuentasegreso, $nivelinit, $nivelfin)
{
    ...
    $pdf->MultiCell(0, 4, strtoupper(decode_utf8('La Infrascrita Perito Contador registrado ante la Superintendencia de Administración Tributaria (SAT) bajo el número de registro trescientos nueve millones doscientos ochenta y seis mil trescientos ochenta y siete (309,286,387). CERTIFICA QUE:  el presente Estado de Resultados que antecede, muestra la Situación Financiera de las operaciones de la entidad. Por el período comprendido ' . $datos[0] . ', de conformidad con los principios de contabilidad generalmente aceptados.')));
    $pdf->Ln(15);
    
    $pdf->firmas(2, ["Angela Rocio López Galindo","Marcelino Tinuar Cipriano"]); 
    $pdf->Ln(12);
    $pdf->CellFit($ancho_linea2 + 15, $tamanio_linea, ' ', '', 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 * 3 + 3, $tamanio_linea, 'PERITO CONTADO', '', 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 * 2, $tamanio_linea, 'TITULAR DE PROGRAMA', '', 0, 'L', 0, '', 1, 0);
    $pdf->Ln(4);
    $pdf->CellFit($ancho_linea2 + 8, $tamanio_linea, ' ', '', 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 * 3, $tamanio_linea, 'REGISTRO No. 309286387', '', 0, 'L', 0, '', 1, 0);
