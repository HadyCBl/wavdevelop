...
$strquery = "SELECT ccodcta,id_ctb_nomenclatura,SUM(debe) sumdeb, SUM(haber) sumhab from ctb_diario_mov 
    ...
    AND substr(ccodcta,1,1) IN (SELECT clase FROM ctb_parametros_cuentas WHERE (id_tipo>=1 AND id_tipo<=3) || id_tipo=6) $condi
...
try {
    ...
    WHERE estado=1 AND substr(ccodcta,1,1) IN (SELECT clase FROM ctb_parametros_cuentas WHERE (id_tipo>=1 AND id_tipo<=3) || id_tipo=6) AND LENGTH(ccodcta) <=13 
    ORDER BY ccodcta;";
...
function printpdf($registro, $datos, $info, $nivelinit, $nivelfin,  $cuentasactivo, $cuentaspasivo, $cuentascapital, $reguladorasactivo, $cuentacontableer, $resultadoer)
{
    ...
    class PDF extends FPDF
    {
        ...
        function Header()
        {
            ...
            $this->Cell(0, 5, 'BALANCE GENERAL RESUMEN ' . $this->datos[0], 0, 1, 'C', true);
            ...
            $monbal = ['Nivel 1', 'Nivel 2', 'Nivel 3', 'Nivel 4', 'Nivel 5', 'Nivel 6', 'Nivel 7'];
            $niv = $this->nivelfin;
    ...
    while ($f < count($cuentas)) {
        ...
        if ($monto != 0) {
            $monbal = [$nivel1, $nivel2, $nivel3, $nivel4, $nivel5, $nivel6, $nivel7];
    ...
    $opResult = array(
        ...
        'namefile' => "Balance General Resumen",
        'tipo' => "pdf",
...
function printxls($registro, $datos,  $cuentasactivo, $cuentaspasivo, $cuentascapital, $reguladorasactivo, $cuentacontableer, $resultadoer)
{
    ...
    $activa->setCellValue('C1', 'NIVEL 7');
    $activa->setCellValue('D1', 'NIVEL 6');
    $activa->setCellValue('E1', 'NIVEL 5');
    $activa->setCellValue('F1', 'NIVEL 4');
    $activa->setCellValue('G1', 'NIVEL 3');
    $activa->setCellValue('H1', 'NIVEL 2');
    $activa->setCellValue('I1', 'NIVEL 1');
    ...
    while ($f < count($cuentas)) {
        ...
        if ($monto != 0) {
            ...
            $activa->setCellValue('C' . $i, $nivel7);
            $activa->setCellValue('D' . $i, $nivel6);
            $activa->setCellValue('E' . $i, $nivel5);
            $activa->setCellValue('F' . $i, $nivel4);
            $activa->setCellValue('G' . $i, $nivel3);
            $activa->setCellValue('H' . $i, $nivel2);
            $activa->setCellValue('I' . $i, $nivel1);
            //***************SUMATORIAS*********************
            $result = array_search($cuenta, array_column($cuentasactivo, 'clase'));
            if ($result !== false) {
                $totalactivo = $totalactivo + $monto;
