...
$condi = ($ragencia == "anyofi") ? " AND id_agencia=?" : "";
...
$strquery = "SELECT dia.numdoc,mov.id_ctb_nomenclatura,cue.ccodcta,cue.cdescrip,dia.feccnt,dia.numcom,dia.glosa,mov.debe,mov.haber,mov.id_ctb_diario,IFNULL(chq.nomchq ,'-') AS nombrecheque
                FROM ctb_diario dia 
                ...
                WHERE dia.estado=1 AND feccnt BETWEEN ? AND ? $condi 
...
$num_registros = count($ctbmovdata);
...
switch ($tipo) {
    case 'xlsx';
        printxls($ctbmovdata, [$flag, 0], $num_registros);
        break;
    case 'pdf':
        printpdf($ctbmovdata, [$titlereport, $flag, 0], $info, $num_registros);
        break;
}
...
function printpdf($registro, $datos, $info, $num_registros)
{
    ...
    $pdf->SetFont($fuente, 'B', 10); // Ajusta la fuente y el tamaño para que coincidan con el resto del documento
