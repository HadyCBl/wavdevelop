...
function printpdf($registro, $datos, $info, $cuentasingreso, $cuentasegreso, $nivelinit, $nivelfin)
{
    ...
    $sum_ing_eg = number_format($totali - $totalg, 2, '.', ',');
    $format_monto = new NumeroALetras();
    $monto_total = $totali - $totalg;
    $es_negativo = $monto_total < 0;
    $monto_abs = abs($monto_total);
    $decimal = explode(".", number_format($monto_abs, 2, '.', ''));
    $decimal[1] = $decimal[1] ?? '00';
    $res = ($decimal[1] == '00') ? '00' : $decimal[1];
    $signo_texto = $es_negativo ? 'MENOS ' : '';
    $resultado_letras = $signo_texto . $format_monto->toMoney($decimal[0]) . ' QUETZALES CON ' . $res . "/100";
    $pdf->SetFont($fuente, 'B', 6);
    $pdf->MultiCell(0, 4, strtoupper(Utf8::decode('  EL INFRASCRITO PERITO CONTADOR DIEGO ARNOLDO RAMÍREZ RAYMUNDO CON NUMERO 80251919 AUTORIZADO POR LA SUPERINTENDENCIA DE ADMINISTRACION TRIBUTARIA(SAT), CERTIFICA: QUE EL PRESENTE ESTADO DE RESULTADOS PRACTICADO ' . $datos[0] . ', DE LA ASOCIACION DE MUJERES MAYAS IXILES "AMMI", UBICADA EN EL CANTON BATZBACA, MUNICIPIO DE NEBAJ, DEPARTAMENTO DE QUICHÉ; EL CUAL ASCIENDE A LA CANTIDAD DE: ' . $resultado_letras . '(Q.' . $sum_ing_eg . ').')));
    $pdf->firmas(2, ["P.C. Diego Arnoldo Ramírez Raymundo", " Lic. Francisco  Rubén Ceto Guzmán"]);

    $pdf->SetFont($fuente, 'B', 8);
    $pdf->Ln(12);
    $pdf->CellFit($ancho_linea2 - 10, $tamanio_linea, ' ', '', 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 * 3 + 5, $tamanio_linea, 'Contador: Asociacion de Mujeres Mayas Ixiles', '', 0, 'L', 0, '', 1, 0);
    $pdf->CellFit($ancho_linea2 - 20, $tamanio_linea, ' ', '', 0, 'L', 0, '', 1, 0);
    $pdf->CellFit(0, $tamanio_linea, 'Gerente: Asociacion de Mujeres Mayas Ixiles', '', 0, 'L', 0, '', 1, 0);

    $pdf->firmas(1, ["Licda. Jacinta Pérez Cobo de Ceto"]);
    $pdf->Ln(12);
    $pdf->CellFit(0, $tamanio_linea, 'Presidenta y Representante Legal Asociacion de Mujeres Mayas Ixiles', '', 0, 'C', 0, '', 1, 0);
