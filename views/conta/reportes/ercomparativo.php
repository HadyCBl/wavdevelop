...
if ($inputs[0] > $inputs[1]) {
    echo json_encode(['status' => 0, 'mensaje' => 'Rango de fechas inválido para el ER 1']);
    return;
}
if ($inputs[2] > $inputs[3]) {
    echo json_encode(['status' => 0, 'mensaje' => 'Rango de fechas inválido para el ER 2']);
    return;
}
if ($inputs[0] == $inputs[2] && $inputs[1] == $inputs[3]) {
    echo json_encode(['status' => 0, 'mensaje' => 'La fecha de los dos ER son iguales']);
    return;
}
if ($inputs[1] > $hoy || $inputs[3] > $hoy) {
    echo json_encode(['status' => 0, 'mensaje' => 'La fecha final del ER no puede ser mayor a la de hoy']);
    return;
}
...
if ($anioini1 != $aniofin1) {
    echo json_encode(['status' => 0, 'mensaje' => 'El rango de fechas del Primer ER tienen que ser del mismo año']);
    return;
}
...
if ($anioini2 != $aniofin2) {
    echo json_encode(['status' => 0, 'mensaje' => 'El rango de fechas del Segundo ER tienen que ser del mismo año']);
    return;
}

/*  ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    +++++++++++++++++++++++++++ CONSULTA FINAL PRIMER ER +++++++++++++++++++++++++++++
    ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
$query = "SELECT ccodcta,id_ctb_nomenclatura,SUM(debe) sumdeb, SUM(haber) sumhab from ctb_diario_mov 
    WHERE estado=1 AND substr(ccodcta,1,1) IN (SELECT clase FROM ctb_parametros_cuentas WHERE id_tipo>=4 AND id_tipo<=5) 
...
$er1 = $response[0];
if (count($er1) == 0) {
    echo json_encode(['status' => 0, 'mensaje' => 'No hay datos para el primer ER']);
    return;
}


// $query = "SELECT ccodcta,id_ctb_nomenclatura,SUM(debe) sumdeb, SUM(haber) sumhab from ctb_diario_mov 
// WHERE estado=1 AND substr(ccodcta,1,1)>=6 AND substr(ccodcta,1,1)<=7 AND feccnt BETWEEN ? AND ? GROUP BY ccodcta ORDER BY ccodcta";
// $response = executequery($query, [$inputs[0], $inputs[1]], $conexion);
// if (!$response[1]) {
//     echo json_encode(['status' => 0, 'mensaje' => $response[0]]);
//     return;
// }
...
$er2 = $response[0];
if (count($er2) == 0) {
    echo json_encode(['status' => 0, 'mensaje' => 'No hay datos para el segundo ER']);
    return;
}

// $query = "SELECT ccodcta,id_ctb_nomenclatura,SUM(debe) sumdeb, SUM(haber) sumhab from ctb_diario_mov WHERE estado=1 AND substr(ccodcta,1,1)>=6 AND substr(ccodcta,1,1)<=7 AND feccnt BETWEEN ? AND ? GROUP BY ccodcta ORDER BY ccodcta";
// $response = executequery($query, [$inputs[2], $inputs[3]], $conexion);
// if (!$response[1]) {
//     echo json_encode(['status' => 0, 'mensaje' => $response[0]]);
//     return;
// }
...
$query = "SELECT id,ccodcta,cdescrip from ctb_nomenclatura 
    WHERE estado=? AND substr(ccodcta,1,1) IN (SELECT clase FROM ctb_parametros_cuentas WHERE id_tipo>=4 AND id_tipo<=5) ORDER BY ccodcta";
...
// $query = "SELECT id,ccodcta,cdescrip from ctb_nomenclatura WHERE estado=1 AND substr(ccodcta,1,1)>=6 AND substr(ccodcta,1,1)<=7 AND 's'=? ORDER BY ccodcta";
// $response = executequery($query, ['s'], $conexion);
// if (!$response[1]) {
//     echo json_encode(['status' => 0, 'mensaje' => $response[0]]);
//     return;
// }
...
if (!$flag) {
    echo json_encode(['status' => 0, 'mensaje' => 'No hay cuentas configuradas para el calculo del ER']);
    return;
}
...
switch ($tipo) {
    case 'xlsx';
        printxls($er1, $er2, $info, $cuentas, $inputs, $parametros, $cuentasingreso, $cuentasegreso);
        break;
    case 'pdf':
        printpdf($er1, $er2, $info, $cuentas, $inputs, $parametros, $cuentasingreso, $cuentasegreso);
        break;
}

//funcion para generar pdf
function printpdf($er1, $er2, $info, $cuentas, $inputs, $parametros, $cuentasingreso, $cuentasegreso)
{
    ...
    class PDF extends FPDF
    {
        ...
        function Header()
        {
            ...
            $this->Cell(0, 5, 'ESTADO DE RESULTADOS COMPARATIVO ', 0, 1, 'C', true);
    ...
    while ($f < count($cuentas)) {
        ...
        $debe1 = opcalcular($er1, $account, 'sumdeb', $nivel, $opcion);
        $haber1 = opcalcular($er1, $account, 'sumhab', $nivel, $opcion);

        $debe2 = opcalcular($er2, $account, 'sumdeb', $nivel, $opcion);
        $haber2 = opcalcular($er2, $account, 'sumhab', $nivel, $opcion);

        // $saldo1 = ($clase == 6) ? $debe1 - $haber1 : $haber1 - $debe1;//OPCION 1
        // $saldo1 = ($clase == 7) ? $debe1 - $haber1 : $haber1 - $debe1; //OPCION 2
        ...
        // $saldo2 = ($clase == 6) ? $debe2 - $haber2 : $haber2 - $debe2; //OPCION 1
        // $saldo2 = ($clase == 7) ? $debe2 - $haber2 : $haber2 - $debe2; //OPCION 2
...
function printxls($balance1, $balance2, $info, $cuentas, $inputs, $parametros, $cuentasingreso, $cuentasegreso)
{
    require '../../../vendor/autoload.php';

    $excel = new Spreadsheet();
    $activa = $excel->getActiveSheet();
    $activa->setTitle("ER_Comparativo");
    ...
    $activa->setCellValue('C1', 'ER 1');
    $activa->setCellValue('D1', 'ER 2');
