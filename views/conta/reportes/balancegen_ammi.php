...
function printpdf($registro, $datos, $info, $nivelinit, $nivelfin,  $cuentasactivo, $cuentaspasivo, $cuentascapital, $reguladorasactivo, $cuentacontableer, $resultadoer)
{
    ...
    $sum_pas_cap = number_format($totalpasivo + $totalcapital, 2, '.', ',');
    $format_monto = new NumeroALetras();
    $decimal = explode(".", ($totalpasivo + $totalcapital));
    $decimal[1]  ??= 0;
    $res = ($decimal[1] == 0) ? 0 : $decimal[1];
    // $sum_activo_letras = $format_monto->toMoney($decimal[0], 2, '', '') . 'QUETZALES CON ' . $res . "/100 ";
    $sum_activo_letras = $format_monto->toMoney($decimal[0], 2, '', '') . 'QUETZALES CON ' . $res . "/100 ";

    $pdf->Ln(4);
    $pdf->MultiCell(0, 4, strtoupper(decode_utf8('  EL INFRASCRITO PERITO CONTADOR DIEGO ARNOLDO RAMÍREZ RAYMUNDO CON NUMERO 80251919 AUTORIZADO POR LA SUPERINTENDENCIA DE ADMINISTRACION TRIBUTARIA(SAT), CERTIFICA: QUE EL PRESENTE BALANCE GENERAL PRACTICADO ' . $datos[0] . ';  DE LA ASOCIACION DE MUJERES MAYAS IXILES "AMMI", UBICADA EN EL CANTON BATZBACA, MUNICIPIO DE NEBAJ, DEPARTAMENTODE QUICHÉ; EL CUAL ASCIENDE A LA CANTIDAD DE; '. $sum_activo_letras .'(Q.'. $sum_pas_cap .').')));
    $pdf->Ln(7);
