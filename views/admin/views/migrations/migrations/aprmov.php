...
if (isset($_GET['listen'])) {
    ...
    try {
        ...
        foreach ($datos as $key => $value) {
             ...
             */
            $cuenta = $database->selectColumns("aprcta", ["ccodaport", "nlibreta"], "$campoCuentaDestino=?", [$value[$campoCuentaAnfitrion]]);
            if (empty($cuenta)) {
                $conterrors++;
                sendSSEMessage('progress', [
                    'row' => $key + 1,
            ...
            $ahommov = array(
                ...
                "monto" => $value['monto'] ?? 0,
                "cuota_ingreso" => $value['cuota_ingreso'] ?? 0,
            ...
            $database->insert("aprmov", $ahommov);
            $contaux++;
            $contadorSeguimiento++;
