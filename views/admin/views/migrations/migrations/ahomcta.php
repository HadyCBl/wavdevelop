...
if (isset($_GET['listen'])) {
    ...
    try {
        ...
        foreach ($datos as $key => $value) {
             ...
             */
            $producto = $database->selectColumns("ahomtip", ["ccodtip", "ccodofi", "tasa"], "$campoProductoDestino=?", [$value[$campoProductoAnfitrion]]);
            if (empty($producto)) {
                $conterrors++;
            ...
            $idAgenciaCuenta = (int) $producto[0]['ccodofi'];
            $ress = $database->getAllResults("SELECT aho_ccodaho(?,?) ccodaho", [$idAgenciaCuenta, $producto[0]['ccodtip']]);
            if (empty($ress)) {
                $conterrors++;
                sendSSEMessage('progress', [
                    'row' => $key + 1,
            ...
            $ahomcta = array(
                "ccodaho" => $value['ccodaho'] ?? $codcuenta,
                "ccodcli" => $cliente[0]['idcod_cliente'],
                "num_nit" => $value["num_nit"] ?? '',
                "ccodcli2" => $value["ccodcli2"] ?? '',
                ...
                "cnomaho" => $value["cnomaho"] ?? "",
                "accountprg" => $value["accountprg"] ?? '',
                "monobj" => $value["monobj"] ?? NULL,
                "dep" => 1,
                "ret" => 1,
                "fecini" => NULL,
                "fecfin" => NULL,
