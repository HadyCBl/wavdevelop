...
function impresion_libreta_cope27(datos, resta, ini, posini, posfin, saldoo, numi, file) {
    //console.log(datos);
    //FRONT_INI = 59  -- NUMFRONT = 27  ---  NUMDORS - 26
    var inif = parseInt(datos[0][2]);
    var nfront = parseInt(datos[0][0]);
    var inid = parseInt(datos[0][3]);
    var ndors = parseInt(datos[0][1]);
    ...
    while (i < (datos[1].length)) {
        ...
        doc.text(28, pos, datos[1][i]['id_agencia']); // AGENCIA
...
function impresion_recibo_dep_ret_coopeadg(datos) {
    ...
    var montol = datos[11];
    var agencia = (datos[22] == 1) ? "Tecpán Guatemala, Chimaltenango. " : "San José Poaquil ";
    if (recibos == 'D') {
...
function impresion_certificado_credivasquez(datos) {
    ...
    doc.text(x + 65, y, datos[3]); //CODIGO DEL CLIENTE
    doc.text(x + 45, y + 8, nomcli); //nombre del cliente
    doc.text(x + 175, y + 8, datos[0][4]); //Dpi

    doc.text(x + 50, y + 18, datos[0][3]); //Direccion
    doc.text(x + 65, y + 28, 'Q.' + intmonto);//monto +15 de texto
    doc.text(x + 95, y + 28, montoEnLetras);// monto en letras
    doc.text(x + 40, y + 38, datos[0][8] + ' días.'); //PLAZO
    doc.text(x + 70, y + 48, fechafin);//FECHA vence
    doc.text(x + 55, y + 58, intere + '% Anual');//interes
    ...
    var ini = 118;
    while (i < datos[1].length) {
        doc.text(35, ini, datos[1][i]['nombre']);
        doc.text(170, ini, datos[1][i]['codparent']);
        doc.text(35, ini + 10, datos[1][i]['dpi']);
...
function impresion_certificado_cope27(datos) {
    ...
    doc.text(241, harvey + 8, tasaInteres); //tasa de interes anual
    //tercera fila
    doc.text(60, harvey + 18, datos[0][3]); //address
    doc.text(230, harvey + 18, datos[0][5]);
    //cuarta fila
    doc.text(60, harvey + 26, datos[0][6]); // total en letras
    let montoFormateado = Number(datos[0][7]).toLocaleString('es');
    doc.text(150, harvey + 26, '****Q.' + montoFormateado + '****');

    doc.text(230, harvey + 26, datos[0][8]); // plazo
    //quinta fila
    doc.text(60, harvey + 34, conviertefecha(datos[0][9])); // fecha apertura
    doc.text(247, harvey + 34, conviertefecha(datos[0][10])); // fecha vence
    doc.text(45, harvey + 90, 'Totonicapán, ' + datosCuenta['fechaletra']);
    ...
    while (i < datos[1].length) {
        ...
        doc.text(109, ini, '  ' + datos[1][i]['direccion']);
        doc.text(205, ini, '  ' + datos[1][i]['dpi']);
        doc.text(248, ini, '  ' + datos[1][i]['porcentaje'] + ' %');
...
function impresion_certificado_coinco(datos) {
    ...
    let monto = isNaN(datosCuenta['montoapr']) ? 0 : datosCuenta['montoapr'];
    let montoApertura = Number(monto).toLocaleString('en-US');

    const y = 67;
    const x = 10;

    doc.text(x + 50, y, datosCuenta['nombre']);
    doc.text(x + 43, y + 5, datosCuenta['codaho']);
    doc.text(x + 40, y + 19, datosCuenta['fechaletra']);
    doc.text(x + 35, y + 23, datosCuenta['nombre']);
    doc.text(x + 35, y + 31, datosCuenta['dpi']);
    doc.text(x + 22, y + 40, datosCuenta['tel']);
    doc.text(x + 47, y + 50, montoApertura);
    doc.text(x + 173, y + 50, datosCuenta['plazo'] + ' días.');
    doc.text(x + 45, y + 58, (datosCuenta['calint'] == "V") ? 'VENCIMIENTO' : 'MENSUAL');
    doc.text(x + 40, y + 68, conviertefecha(datosCuenta['fecapr']));
    doc.text(x + 131, y + 68, conviertefecha(datosCuenta['fec_ven']));

    var beneficiarios = "";

    for (let i = 1; i < datos[1].length; i++) {
        beneficiarios = beneficiarios + datos[1][i]['nombre'] + " [" + datos[1][i]['codparent'] + "]";
        if (i !== ((datos[1].length) - 1)) {
            beneficiarios = beneficiarios + ", ";
        }
    }

    doc.text(x + 38, y + 77, beneficiarios);
...
function impresion_certificado_copim(datos) {
    ...
    doc.setFont('helvetica', 'normal');
    doc.text(x + 5, y + 15, datosCuenta['codaho']); //cuenta
    doc.text(x + 90, y + 15, datosCuenta['dpi']); //Dpi

    doc.text(x + 10, y + 22, datosCuenta['nombre']); //nombre del cliente
    doc.text(x + 167, y + 22, intere + '%');//interes

    doc.text(x + 10, y + 28, datosCuenta['dire']); //Direccion
    doc.text(x + 140, y + 28, datosCuenta['tel']); //telefono

    doc.text(x, y + 34, datosCuenta['montoletra']);// monto en letras
    doc.text(x + 85, y + 34, intmonto);//monto
    doc.text(x + 155, y + 34, datosCuenta['plazo'] + ' días.'); //PLAZO

    doc.text(x + 5, y + 40, fechaini);//FECHA INICIO
    doc.text(x + 170, y + 40, fechafin);//FECHA vence
    doc.text(x, y + 79, "Coban, Alta Verapaz, " + datosCuenta['fechaletra']); //Fecha de impresion
...
function impresion_recibo_dep_ret_kotan(datos) {
    ...
    while (i < 2) {
        ...
        ini = ini + 4;
        doc.text(margenizquierdo + 42, ini, 'GERETE GENERAL');
