...
class Aprmov
{
    ...
    public function applyDeposito($datos)
    {
        ...
        try {
            ...
            $aprctaModel = new \Micro\Models\Aprcta($this->db);
            $nlibreta = $aprctaModel->getCurrentLibretaNumber($datos['cuenta_id']);

            $datos['monto'] = $datos['monto'] ?? 0.00;

            $aprmov = array(
                "ccodaport" => $datos['cuenta_id'],
                "dfecope" => $datos['fecha'],
                "ctipope" => "D",
            ...
            $id_movimiento = $this->db->insert('aprmov', $aprmov);

            $idCuentaContable = $aprctaModel->getAccountContable($datos['cuenta_id']);
             ...
             */
            $this->db->executeQuery('CALL apr_ordena_noLibreta(?,?);', [$nlibreta, $datos['cuenta_id']]);
            $this->db->executeQuery('CALL apr_ordena_Transacciones(?);', [$datos['cuenta_id']]);
