...
/**
 * SEGUNDA VERSION CON PDO CONTROLADO
 */
class DatabaseServiceBackup
{
    ...
    private function backupTables()
    {
        ...
        // Obtener solo las tablas (no vistas)
        $stmt = $this->pdo->query("
            SELECT TABLE_NAME 
        ...
        foreach ($tables as $table) {
            ...
            // Obtener datos de la tabla en chunks para no agotar memoria
            $sql .= $this->backupTableData($table);
        }

        return $sql;
    }

    private function backupTableData($table)
    {
        $sql = '';
        $chunkSize = 500; // Reducir chunk size para menos memoria
        $offset = 0;
        $hasData = false;
        
        try {
            // Primero verificar si la tabla tiene datos
            $countStmt = $this->pdo->query("SELECT COUNT(*) as total FROM `$table`");
            $totalRows = $countStmt->fetch()['total'];
            $countStmt = null; // Liberar memoria
            
            if ($totalRows == 0) {
                return $sql;
            }
            
            $sql .= "-- Datos para la tabla `$table` (Total: $totalRows registros)\n";
            
            // Obtener las columnas de la tabla para optimizar la consulta
            ...
            while (true) {
                // Evitar placeholders para LIMIT/OFFSET en algunas versiones de MySQL/MariaDB
                $limit = (int) $chunkSize;
                $off = (int) $offset;

                $query = "SELECT $columnsList FROM `$table` LIMIT $limit OFFSET $off";
                ...
                if (!$hasData) {
                    // Iniciar la sentencia INSERT una sola vez
                    $sql .= "INSERT INTO `$table` ($columnsList) VALUES\n";
                    $hasData = true;
                } else {
                    // Añadir separador entre lotes
                    $sql .= ",\n";
                }
                ...
                // Liberar memoria del statement y variables
                $stmt = null;
                unset($rows, $insertRows, $values);

                // Force garbage collection cada 5 chunks
                if ($offset % 2500 == 0) {
                    gc_collect_cycles();
                }
            }
            ...
            $sql .= "-- Error al obtener datos de la tabla $table: " . $e->getMessage() . "\n\n";
            Log::error("Error al obtener datos de la tabla $table: " . $e->getMessage());
        }
    ...
    private function backupViews()
    {
        ...
        try {
            ...
            Log::error("Error al obtener vistas: " . $e->getMessage());
        }
    ...
    private function backupFunctions()
    {
        ...
        try {
            ...
            Log::error("Error al obtener funciones: " . $e->getMessage());
        }
    ...
    private function backupProcedures()
    {
        ...
        try {
            ...
            Log::error("Error al obtener procedimientos: " . $e->getMessage());
        }
    ...
    private function backupTriggers()
    {
        ...
        try {
            ...
            Log::error("Error al obtener triggers: " . $e->getMessage());
        }
    ...
    public function generateFullBackupContent()
    {
        try {
            // Usar output buffering para manejar el streaming
            ob_start();
            
            echo $this->getBackupHeader();
            ob_flush();
            flush();
            
            echo $this->backupTables();
            ob_flush();
            flush();
            
            echo $this->backupViews();
            ob_flush();
            flush();
            
            echo $this->backupFunctions();
            ob_flush();
            flush();
            
            echo $this->backupProcedures();
            ob_flush();
            flush();
            
            echo $this->backupTriggers();
            ob_flush();
            flush();
            
            echo $this->getBackupFooter();
            ...
            ob_end_clean();
            throw new Exception("Error al generar el contenido del backup: " . $e->getMessage());
            Log::error("Error al generar el contenido del backup: " . $e->getMessage());
        }
    }

    public function streamFullBackup()
    {
        try {
            // Para streaming directo sin cargar en memoria
            echo $this->getBackupHeader();
            flush();
            ...
            Log::error("Error al generar el streaming del backup: " . $e->getMessage());
            throw new Exception("Error al generar el streaming del backup: " . $e->getMessage());
        }
    }
}
