 ...
 */
class CalculoPagosSemanales
{
    private $montoCredito;
    private $tasaInteres;
    private $numPeriodos;
    private $tipoTasa;
    private $diasCorridos = 0;
    private $diasDiferencia = 0;
    private $gastos = [];
    private $fechas = [];
     ...
     * @param int $diasCorridos Indica si se usan días corridos. Valor por defecto es 0.
     * @param int $diasDiferencia Diferencia de días. Valor por defecto es 0.
     ...
     */
    public function __construct($montoCredito, $tasaInteres, $numPeriodos, $tipoTasa = 10, $diasCorridos = 0, $diasDiferencia = 0, $gastos = [], $fechas = [])
    {
    ...
    public function generarTablaAmortizacion()
    {
        ...
        $interesSemanal = ($this->montoCredito * ($this->tasaInteres / 100)) / $factorInteres;
        $ganancia = ($this->tipoTasa == 10) ? round(($capital * $this->tasaInteres / 100), 2) : round($interesSemanal * $this->numPeriodos, 2);

        $precision = $this->getPrecision();
        $mode = $this->getModePrecision();

        $masPrimeraCuota = 0;
        if ($this->diasCorridos == 1 && $this->diasDiferencia > 7) {
            $masPrimeraCuota = ($capital * $this->getTasaDiaria() / 100 * ($this->diasDiferencia - 7));
        }

        $capitalPorCuota = round($this->montoCredito / $this->numPeriodos, 2);
        $saldoCapital = $this->montoCredito;
        $saldoint = $ganancia + $masPrimeraCuota;

        for ($semana = 1; $semana <= $this->numPeriodos; $semana++) {
            $key = $semana - 1; // Índice base 0
            
            $interesCuotaSemanal = ($semana == 1) ? $interesSemanal + $masPrimeraCuota : $interesSemanal;
            $saldoCapital -= $capitalPorCuota;
            $saldoint = $saldoint - $interesCuotaSemanal;

            // Ajuste de saldo de interés en última cuota
            if ($semana == $this->numPeriodos && $saldoint != 0) {
                $interesCuotaSemanal = $interesCuotaSemanal + $saldoint;
            }

            // Ajuste de saldo de capital en última cuota
            $capitalCuota = $capitalPorCuota;
            if ($semana == $this->numPeriodos && $saldoCapital != 0) {
            ...
            $result[$key] = [
                'nrocuota' => $semana,
                'fecha' => $fechaCuota,
                'cuota' => $capitalCuota + $interesCuotaSemanal,
                'interes' => round($interesCuotaSemanal, 2),
            ...
            if (!empty($this->gastos)) {
                $gastosAplicados = $this->aplicarGastos($key, $capitalCuota, $interesCuotaSemanal);
            ...
            // Aplicar precisión y ajustes (para créditos semanales - 7D)
            ...
            if ($ajuste != 0) {
                ...
                if (!empty($result[$key]['gastos'])) {
                    ...
                    // $result[$key]['gastos'][0]['monto'] = round($result[$key]['gastos'][0]['monto'] + $ajuste, $precision, $mode);
                    $result[$key]['gastos'][0]['monto'] = $result[$key]['gastos'][0]['monto'] + $ajuste;
                } else {
                    // Si no hay gastos, aplicar en intereses
                    // $result[$key]['interes'] = round($result[$key]['interes'] + $ajuste, $precision, $mode);
    ...
    private function getTasaDiaria()
    {
        switch ($this->tipoTasa) {

            case 9: // Anual

                return $this->tasaInteres / 360;
            case 5: // Mensual
                return $this->tasaInteres / 30;
            case 10: // Sobre el plazo
                return $this->tasaInteres / $this->numPeriodos / 7;
            default:
